...
describe('onDispose 里的持久化(整体验收修复)', () => {
  it('lets a plugin persist state from onDispose — the most natural teardown there is', () => {
    /*
     * `disposeCorePluginState` 上一版第一句就置 `disposed = true`,**然后**才 drain
     * onDispose 回调;而 app 层还把 `store.dispose()` push 在回调数组的最前面。
     * 于是插件最自然的收尾写法(onDispose 里存盘)必定撞上闩:数据静默丢失,
     * 而报的错("插件已拆除")还会把排查引到错误的方向。
     *
     * 内置插件恰好只在 onDispose 里关流,所以全套测试都是绿的 —— 这条只有
     * 第三方插件会踩,而它一定会踩。
     */
    const root = tempRoot()
    try {
      ...
      const state = {
        ...
        unsubs: [], commands: new Map(), requestHandlers: new Map(), toolIds: [],
        skillRootUnsubs: [], promptContextUnsubs: [], lifecycleUnsubs: [],
        configUnsubs: [], disposeCallbacks: [] as Array<() => void>,
        disposed: false,
        disposing: false,
      }
      ...
      state.disposeCallbacks.push(() => {
        try {
          storage.writeJson('final.json', { closedAt: 1 })
          persisted = true
        } catch (error) {
          failure = error
        }
      ...
      disposeCorePluginState(state as never)

      expect(failure, `onDispose write must not be rejected: ${String(failure)}`).toBeUndefined()
      expect(persisted).toBe(true)
      expect(storage.readJson('final.json')).toEqual({ closedAt: 1 })
      // 窗口关上之后照旧拒绝 —— 放行的只是 dispose 期间。
      expect(state.disposed).toBe(true)
      expect(state.disposing).toBeFalsy()
    } finally {
      fs.rmSync(root, { recursive: true, force: true })
    }
  })

  it('still refuses a write that arrives after teardown finished', () => {
    const root = tempRoot()
    try {
      const state = {
        api: undefined as never,
