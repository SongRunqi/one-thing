...
function codexEncryptedReasoningToProviderData(values: string[] | undefined): AgentProviderData[] {
  return (values ?? [])
    .filter((value): value is string => typeof value === 'string' && value.length > 0)
    .map(encryptedContent => ({
      provider: 'codex' as const,
      type: 'encrypted-reasoning' as const,
      encryptedContent,
    }))
}
...
export function agentMessagesFromHistory(messages: AgentHistoryMessage[]): AgentMessage[] {
  ...
  for (const message of messages) {
    ...
    if (message.role === 'assistant') {
      ...
      const providerData = codexEncryptedReasoningToProviderData(message.codexEncryptedReasoning)
      result.push({
        role: 'assistant',
