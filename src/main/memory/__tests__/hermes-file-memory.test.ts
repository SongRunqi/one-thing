...
function makeWorkspace(): MemoryWorkspace {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'hermes-file-memory-'))
  tempDirs.push(root)
  return {
    settings: normalizeSoulMemorySettings() as ResolvedSoulMemorySettings,
    agentId: 'default',
    root,
    ...
    todayPath: path.join(root, 'memory', '2026-06-10.md'),
...
describe('Hermes file memory', () => {
  ...
  it('builds a bounded prompt snapshot from USER.md and MEMORY.md', async () => {
    ...
    expect(status.files.find(file => file.target === 'user')?.entries).toBe(1)
    expect(status.files.find(file => file.target === 'memory')?.entries).toBe(1)
  })
})
