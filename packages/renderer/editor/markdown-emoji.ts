...
/* Status emoji arrive from model output as full-color bitmaps — the only
   saturated pixels on the paper surface. Chat display maps them to ink
   glyphs that inherit text color; source text (editor, copies) is never
   mutated. */
const STATUS_EMOJI_GLYPHS: Array<[RegExp, string]> = [
  [/✅/g, '✓'], // ✅ white heavy check mark
  [/✔️?/g, '✓'], // ✔️ heavy check mark
  [/❌/g, '✗'], // ❌ cross mark
  [/❎/g, '✗'], // ❎ negative squared cross mark
  [/⚠️/g, '⚠'], // ⚠️ → text-presentation warning sign
]

export function normalizeStatusEmoji(text: string): string {
  let result = text
  for (const [pattern, glyph] of STATUS_EMOJI_GLYPHS) {
