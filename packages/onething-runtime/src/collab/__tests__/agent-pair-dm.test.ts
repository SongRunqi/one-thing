...
import { COLLAB_ROOM_TOOLS, resolveCollabToolAllowlist } from '../tool-surface.js'
...
describe('§3.4 工具面:双人房走群房那一路,只是多了 dm', () => {
  it('say + board + dm 是同一档,dm 房不 fork 出第二套', () => {
    expect(COLLAB_ROOM_TOOLS).toEqual(['say', 'board', 'dm'])
...
describe('§3.1/D4 双人房情况说明', () => {
  ...
  it('说清三件事:对面是谁、用户在旁观、没有搬运上下文', () => {
    ...
    expect(context).toContain('这是你和阿明(产品)的私聊,只有你们两个人')
    expect(context).toContain('用户看得见这场对话,也随时可能插进来说话')
    expect(context).toContain('对方看不到你在别处的上下文')
    // 沟通场:正经活回大群立卡(§5 规则 3),口径与通用规则一致。
    expect(context).toContain('要动手的正经活回大群立卡再干')
  })

  it('不是群:不念花名册,也不把用户说成群成员', () => {
    const context = buildCollabRoomContext({ ...base, dmPair: true })
    expect(context).not.toContain('群成员')
    expect(context).not.toContain('你在群聊')
  })

  it('群房与单成员私聊的文案零改动', () => {
    const group = buildCollabRoomContext({ self: FE, members: PAIR, roomName: '官网改版组' })
    expect(group).toContain('你在群聊「官网改版组」里,群成员:用户、阿明(产品)')
    const sole = buildCollabRoomContext({ self: FE, members: [FE], roomName: '小李', dm: true })
    expect(sole).toContain('这是你和用户的私聊')
  })
  ...
  it('系统提示词整体:persona 原文 + 双人情况说明 + pair 版通用规则', () => {
    ...
    expect(prompt).toContain('这是你和阿明(产品)的私聊')
    expect(prompt).toContain('你在和另一位同事的私聊里，用户可以旁观也可以插话')
  })
})
