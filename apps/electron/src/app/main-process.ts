...
import { initializeIPC, initializeMCP, shutdownMCP, initializeSkills, initializeACP, shutdownACP } from '@main/ipc/handlers.js'
...
import { sanitizeAllSessionsOnStartup } from '@main/stores/sessions.js'
...
import { configureStorePathHost, getMediaImagesDir } from '@main/stores/paths.js'
import { initializeEventSystem, shutdownEventSystem, getEventBus } from '@main/events/index.js'
import { initializeIPCBridge, shutdownIPCBridge } from '@main/bridges/ipc-bridge-lifecycle.js'
import { initializeSessionLayer, shutdownSessionLayer } from '@main/session/index.js'
...
import { initializeAppLogging, shutdownAppLogging } from '@main/logging/index.js'
import { hydrateProcessEnvFromLoginShell } from '@onething/electron-host/app/login-shell-env'
import { StoreLock, LockConflictError, formatDesktopLockConflict } from '@onething/runtime/storage'
import {
...
async function refreshModelsOnFirstStartup(): Promise<void> {
  ...
  const { refreshAllProviders } = await import('@main/providers/model-registry.js')
...
function createElectronMainWindowOptions(): ElectronActivateOptions {
  return {
    ...
    attachVoiceMainWindow: window => getVoiceService().attachMainWindow(window),
    warmSearchWindow,
    warmTodoPlanWindow: () => warmTodoPlanWindow({
