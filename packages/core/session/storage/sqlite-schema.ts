...
export interface ApplySqliteSchemaMigrationsWithAdaptersOptions {
  migrations?: SqliteMigration[]
  now?: () => number
  hasMigration(version: number): boolean
  applyMigration(migration: SqliteMigration, appliedAt: number): void
}

export interface ApplySqliteSchemaMigrationsWithAdaptersResult {
...
export function applySqliteSchemaMigrationsWithAdapters(
  options: ApplySqliteSchemaMigrationsWithAdaptersOptions,
): ApplySqliteSchemaMigrationsWithAdaptersResult {
  const migrations = options.migrations ?? SESSION_REPOSITORY_MIGRATIONS
  const now = options.now ?? Date.now
  const result: ApplySqliteSchemaMigrationsWithAdaptersResult = {
    appliedVersions: [],
    skippedVersions: [],
  }

  for (const migration of migrations) {
    if (options.hasMigration(migration.version)) {
      result.skippedVersions.push(migration.version)
      continue
    }

    options.applyMigration(migration, now())
