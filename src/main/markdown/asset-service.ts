...
async function fileStat(filePath: string): Promise<import('fs').Stats | null> {
  try {
...
async function markdownContext(documentPath: string, workspaceRoot?: string): Promise<MarkdownContext> {
  ...
  const noteRoot = noteRoots().find(root => isPathInside(root, resolvedDocumentPath))
  if (noteRoot) {
    return {
      type: 'note',
      root: noteRoot,
...
function attachmentDirectoryForContext(context: MarkdownContext): { directory?: string; error?: string; code?: string } {
  const editorSettings = getSettings().general.editor || {}
...
function candidatePaths(context: MarkdownContext, target: string): string[] {
  if (!target) return []
  ...
  if (context.type === 'obsidian') {
    ...
    const configured = getSettings().general.editor?.markdownProjectAttachmentDirectory?.trim()
...
export async function resolveMarkdownAsset(request: MarkdownResolveAssetRequest): Promise<MarkdownAssetResolution> {
  ...
  const context = await markdownContext(request.documentPath, request.workspaceRoot)
  for (const candidate of candidatePaths(context, target)) {
...
export async function saveMarkdownAttachments(
  ...
  const context = await markdownContext(request.documentPath, request.workspaceRoot)
  const target = attachmentDirectoryForContext(context)
  if (!target.directory) {
