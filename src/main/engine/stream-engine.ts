...
export class StreamEngine extends OnethingStreamEngine<EventBus, StreamSender> {
  ...
  override steerMessage(sessionId: string, content: string, source = 'api', origin?: MessageOrigin): void {
    const routed = getChannelSessionRouter().route({
      sessionId,
      origin,
    ...
    super.steerMessage(routed.sessionId, content, source || routed.origin.source, routed.origin)
  }

  override followUpMessage(sessionId: string, content: string, source = 'api', origin?: MessageOrigin): void {
    const routed = getChannelSessionRouter().route({
      sessionId,
      origin,
      fallbackTransport: fallbackTransportForCommand({ source }),
    ...
    super.followUpMessage(routed.sessionId, content, source || routed.origin.source, routed.origin)
  }
...
function fallbackTransportForCommand(command: {
  ...
  if (command.channel && command.channel !== 'ipc' && command.channel !== 'desktop') return 'im'
  return 'desktop'
}
...
function shouldPreserveSessionId(command: {
  source?: string
}): boolean {
