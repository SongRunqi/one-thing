...
function workspaceGrantsPath(): string {
...
function loadWorkspaceGrants(): PermissionGrant[] {
  ...
  workspaceGrantsCache = readJsonFile<WorkspaceGrantFile>(workspaceGrantsPath(), { grants: [] }).grants
  return workspaceGrantsCache
}

function saveWorkspaceGrants(): void {
  writeJsonFile(workspaceGrantsPath(), { grants: workspaceGrantsCache ?? [] })
}
