...
export interface CodexAgentProviderOptions {
  ...
  requestDumper?: (request: CodexAgentProviderRequestDump) => Promise<string | undefined>
}
...
type CodexInputItem =
  | { type: 'message'; role: 'user' | 'assistant' | 'developer'; content: CodexInputContentPart[] }
  | { type: 'function_call'; name: string; arguments: string; call_id: string }
  | { type: 'function_call_output'; call_id: string; output: string | CodexInputContentPart[] }
  | { type: 'reasoning'; summary: AgentJsonValue[]; encrypted_content: string }

type CodexTool =
  | { type: 'function'; name: string; description?: string; strict: false; parameters: AgentJsonObject }
...
async function resolveCodexTokenForRequest(
  ...
  const hasOAuthCredentials = options.authContext?.kind === 'oauth' || Boolean(options.oauthToken)
  if (hasOAuthCredentials && options.refreshOAuthToken) {
    try {
      return await options.refreshOAuthToken(forceRefresh)
    } catch (error) {
      if (forceRefresh) throw error
      console.warn('[CodexAgentProvider] Failed to resolve fresh OAuth token; using provided token snapshot:', error)
    }
  }
...
function canRefreshCodexOAuth(options: CodexAgentProviderOptions): boolean {
  return Boolean(options.refreshOAuthToken) && (options.authContext?.kind === 'oauth' || Boolean(options.oauthToken))
}
...
function dataToUrl(value: string | undefined, mediaType?: string): string | undefined {
  if (!value) return undefined
  if (value.startsWith('data:') || value.startsWith('http://') || value.startsWith('https://')) {
    return value
  }
  return mediaType ? `data:${mediaType};base64,${value}` : value
}
...
function toCodexMessageContent(
  ...
  for (const part of content) {
    switch (part.type) {
      ...
      case 'image':
        if (role === 'user') {
          const imageUrl = dataToUrl(part.image, part.mediaType)
          if (imageUrl) parts.push({ type: 'input_image', image_url: imageUrl, detail: 'auto' })
        }
        break
      case 'file':
        if (role === 'user') {
          const fileData = dataToUrl(part.data, part.mediaType)
          if (fileData) parts.push({ type: 'input_file', file_data: fileData, filename: part.filename })
        }
        break
      default:
        break
...
function codexEncryptedReasoning(message: AgentMessage): string[] {
  return (message.providerData ?? [])
    .filter((data): data is AgentProviderData & { encryptedContent: string } =>
      data.provider === 'codex' &&
  ...
  for (const message of messages) {
    ...
    if (message.role === 'assistant') {
      for (const encryptedContent of codexEncryptedReasoning(message)) {
        input.push({ type: 'reasoning', summary: [], encrypted_content: encryptedContent })
      }
      ...
      for (const toolCall of message.toolCalls ?? []) {
        ...
        input.push({
          type: 'function_call',
          name: toolCall.name,
          arguments: stringifyCodexToolInput(parseToolArguments(toolCall.arguments)),
          call_id: toolCall.id,
        })
      }
      continue
    ...
    if (message.role === 'tool' && message.toolCallId) {
      input.push({
        ...
        output: toolResultToCodexOutput(agentToolMessageContentToStructuredPayload(message.content)),
      })
    }
  }
...
function toolResultToCodexOutput(payload: ReturnType<typeof agentToolMessageContentToStructuredPayload>): string | CodexInputContentPart[] {
  ...
  const rawContent = Array.isArray(payload)
    ...
    : payload && typeof payload === 'object'
      ...
      : undefined
  if (!isAgentToolResultContentPartArray(rawContent)) return JSON.stringify(payload ?? '')
  ...
  for (const part of rawContent) {
    if (part.type === 'text' && typeof part.text === 'string') {
      ...
      const imageUrl = dataToUrl(part.data, part.mimeType)
      if (imageUrl) parts.push({ type: 'input_image', image_url: imageUrl, detail: 'auto' })
    } else if (part.type === 'file') {
      const fileData = dataToUrl(part.data, part.mimeType)
      if (fileData) parts.push({ type: 'input_file', file_data: fileData, filename: part.path })
    }
  }
  return parts.length > 0 ? parts : ''
}

function isAgentToolResultContentPartArray(value: unknown): value is AgentToolResultContentPart[] {
...
function toCodexTools(tools: AgentTool[] | undefined, nativeImageGeneration: boolean): CodexTool[] {
...
function isCodexReasoningModel(model: string): boolean {
  ...
  return lower.startsWith('gpt-5') || lower.includes('codex') || /\bo[13](?:-|$)/.test(lower) || lower.includes('reasoning')
}

function normalizeCodexReasoningEffort(effort: unknown): CodexReasoningOptions['effort'] {
...
function buildCodexRequestBody(request: AgentTurnRequest): CodexRequestBody {
  ...
  const reasoning = request.thinking === 'disabled'
    ? undefined
    : isCodexReasoningModel(request.model) || request.reasoningEffort
      ? { effort: normalizeCodexReasoningEffort(request.reasoningEffort), summary: 'auto' as const }
      : undefined
  return {
    model: request.model,
    instructions: instructions || CODEX_FALLBACK_INSTRUCTIONS,
    input,
    tools: toCodexTools(request.tools, Boolean(request.requestedOutputModalities?.includes('image'))),
...
function summarizeCodexErrorBody(body: string): string {
  ...
  const paragraph = compact.match(/<p>(?:<b>\d+\.<\/b>\s*)?(.*?)(?:<p>|$)/i)?.[1]
    ...
    .replace(/\s+/g, ' ')
    .trim()
  return [title, paragraph].filter(Boolean).join(': ').slice(0, 300) || compact.slice(0, 300)
}

function createCodexAgentApiError(status: number, responseBody: string, headers: Headers): Error {
  const detail = summarizeCodexErrorBody(responseBody)
  const requestId = headers.get('x-oai-request-id')
  return Object.assign(
    new Error(`Codex request failed (${status})${detail ? `: ${detail}` : ''}${requestId ? ` [request-id: ${requestId}]` : ''}`),
    {
      statusCode: status,
      responseBody,
...
function collectReasoningSummaryText(value: AgentJsonValue | undefined): string[] {
...
function collectManyReasoningSummaryText(values: Array<AgentJsonValue | undefined>): string[] {
...
function usageFromResponse(response: CodexSseEvent['response']): AgentUsage | undefined {
...
async function* parseCodexResponsesSse(body: ReadableStream<Uint8Array>): AsyncGenerator<CodexSseEvent> {
  ...
  try {
    ...
    if (buffer.trim()) {
      dataLines.push(buffer.startsWith('data:') ? buffer.slice(5).trimStart() : buffer.trim())
    }
    const event = decode()
    if (event) yield event
  } finally {
...
function summarizeCodexRequestBody(body: CodexRequestBody): Record<string, unknown> {
  const messages = body.input.filter(item => item.type === 'message')
  const lastUser = [...messages].reverse().find(item => item.role === 'user')
  const lastUserText = lastUser?.type === 'message'
    ? lastUser.content.map(part => 'text' in part ? part.text : '').filter(Boolean).join('\n')
    : ''
  return {
    model: body.model,
...
export function createCodexAgentProvider(options: CodexAgentProviderOptions): AgentProvider {
  ...
  async function* streamTurn(request: AgentTurnRequest): AsyncGenerator<AgentTurnStreamEvent, void, void> {
    ...
    const sendRequest = async (token: OAuthToken): Promise<Response> => fetchImpl(requestUrl, {
      method: 'POST',
    ...
    if (response.status === 401 && canRefreshCodexOAuth(options)) {
      ...
      try {
        ...
        if (refreshedToken?.accessToken && refreshedToken.accessToken !== token.accessToken) {
          console.warn('[CodexAgentProvider] Codex request returned 401; refreshed OAuth token and retrying once.')
          response = await sendRequest(refreshedToken)
        } else {
          throw createCodexAgentApiError(response.status, firstBody, response.headers)
        }
      } catch (error) {
        if (error instanceof Error && 'statusCode' in error) throw error
        throw createCodexAgentApiError(401, firstBody || (error instanceof Error ? error.message : String(error)), response.headers)
      }
    }
    ...
    const emitReasoning = function* (delta: string, itemId = 'reasoning-0'): Generator<AgentTurnStreamEvent> {
      if (!delta) return
    ...
    const getFunctionCallCallId = (item: CodexRawRecord, event?: CodexSseEvent): string | undefined => {
      return optionalString(item.call_id) ?? optionalString(item.callId) ?? optionalString(event?.call_id) ?? optionalString(item.id)
    }
    const getFunctionCallToolName = (item: CodexRawRecord): string | undefined => {
      return optionalString(item.name) ?? optionalString(item.tool_name) ?? optionalString(item.toolName)
    }
    ...
    const getReasoningItemId = (item: CodexRawRecord | undefined, event: CodexSseEvent, fallback: string): string => {
      return optionalString(item?.id) ?? optionalString(item?.item_id) ?? optionalString(item?.itemId) ?? optionalString(event.item_id) ?? optionalString(event.itemId) ?? fallback
    }
    const getImageGenerationCallId = (item: CodexRawRecord, event?: CodexSseEvent): string | undefined => {
      return optionalString(item.id) ?? optionalString(item.call_id) ?? optionalString(item.callId) ?? optionalString(event?.item_id) ?? optionalString(event?.itemId)
    }
    ...
    const registerFunctionCallInput = (item: CodexRawRecord, event?: CodexSseEvent): ActiveFunctionCallInput | undefined => {
      ...
      const state = { itemId, callId, toolName, streamedArgs: '', started: false }
      toolInputByItemId.set(itemId, state)
      toolInputByCallId.set(callId, state)
      return state
    }

    const startFunctionCallInput = function* (state: ActiveFunctionCallInput): Generator<AgentTurnStreamEvent> {
      if (state.started) return
      toolCallsEmitted = true
      state.started = true
      yield { type: 'tool-call-start', turn: request.turn, toolCallId: state.callId, toolName: state.toolName }
    }

    const flushPendingFunctionCallDeltas = function* (state: ActiveFunctionCallInput): Generator<AgentTurnStreamEvent> {
      for (const key of Array.from(new Set([state.itemId, state.callId]))) {
        ...
        yield { type: 'tool-call-delta', turn: request.turn, toolCallId: state.callId, toolName: state.toolName, argumentsDelta: delta }
      }
    }

    const registerAndStartFunctionCallInput = function* (item: CodexRawRecord, event?: CodexSseEvent): Generator<AgentTurnStreamEvent> {
    ...
    const emitFunctionCallInputDelta = function* (event: CodexSseEvent): Generator<AgentTurnStreamEvent> {
      const delta = event.delta ?? event.input ?? event.arguments_delta ?? event.argumentsDelta ?? ''
      if (!delta) return
      const itemId = optionalString(event.item_id) ?? optionalString(event.itemId)
      const callId = optionalString(event.call_id) ?? optionalString(event.callId)
    ...
    const emitFunctionCall = function* (item: CodexRawRecord, event?: CodexSseEvent): Generator<AgentTurnStreamEvent> {
      const callId = getFunctionCallCallId(item, event)
      const toolName = getFunctionCallToolName(item)
      ...
      if (state) {
        ...
        if (args && !state.streamedArgs) {
          ...
          yield { type: 'tool-call-delta', turn: request.turn, toolCallId: callId, toolName, argumentsDelta: args }
          ...
          state.streamedArgs = args
          yield { type: 'tool-call-delta', turn: request.turn, toolCallId: callId, toolName, argumentsDelta: suffix }
        }
        toolInputByItemId.delete(state.itemId)
        toolInputByCallId.delete(state.callId)
      } else {
        toolCallsEmitted = true
        yield { type: 'tool-call-start', turn: request.turn, toolCallId: callId, toolName }
        if (args) yield { type: 'tool-call-delta', turn: request.turn, toolCallId: callId, toolName, argumentsDelta: args }
      }
      yield { type: 'tool-call-done', turn: request.turn, toolCall: { id: callId, name: toolName, arguments: args } }
    }
    ...
    for await (const event of parseCodexResponsesSse(response.body)) {
      ...
      if (event.error) throw new Error(`Codex stream error: ${event.error.message ?? 'unknown error'}`)
      ...
      switch (event.type) {
        ...
        case 'response.reasoning_summary_text.delta': {
          const itemId = getReasoningItemId(undefined, event, activeReasoningItemId ?? 'reasoning-0')
          yield* emitReasoning(event.delta ?? event.text ?? '', itemId)
          break
        }
        case 'response.reasoning_summary_part.added': {
          const itemId = getReasoningItemId(undefined, event, activeReasoningItemId ?? 'reasoning-0')
          if (reasoningSummaryByItem.get(itemId)?.trim()) yield* emitReasoning('\n\n', itemId)
          break
        }
        case 'response.reasoning_summary_text.done': {
          ...
          if (summary && !reasoningSummaryByItem.get(itemId)?.trim()) yield* emitReasoning(summary, itemId)
          break
        }
        ...
        case 'response.output_item.added': {
          ...
          if (item.type === 'reasoning') {
            ...
            if (summary && !reasoningSummaryByItem.get(activeReasoningItemId)?.trim()) yield* emitReasoning(summary, activeReasoningItemId)
            break
          }
          ...
          if (item.type === 'image_generation' || item.type === 'image_generation_call') {
        ...
        case 'response.output_item.done': {
            ...
            if (callId && result) {
              yield {
                ...
                providerData: {
                  ...
                  revisedPrompt: optionalString(item.revised_prompt) ?? optionalString(item.revisedPrompt),
                  result,
                },
              }
            }
          ...
          if (item.type === 'reasoning') {
            const itemId = getReasoningItemId(item, event, activeReasoningItemId ?? 'reasoning-0')
            const summary = extractReasoningSummaryText(item)
            if (summary && !reasoningSummaryByItem.get(itemId)?.trim()) yield* emitReasoning(summary, itemId)
            ...
            if (activeReasoningItemId === itemId) activeReasoningItemId = undefined
            break
          }
        ...
        case 'response.incomplete':
          usage = usageFromResponse(event.response) ?? usage
          finishReason = mapCodexFinishReason(event.response?.incomplete_details?.reason)
          break
        case 'response.failed':
          throw new Error(`Codex stream error: ${event.response?.error?.message || 'Codex stream failed'}`)
          break
        default:
          usage = usageFromResponse(event.response) ?? usage
          break
