  ...
  >({
    store: {
      ...
      updateMessageAndTruncate: (sessionId, messageId, newContent, options) =>
        store.updateMessageAndTruncate(sessionId, messageId, newContent, options as Parameters<typeof store.updateMessageAndTruncate>[3]),
      deleteMessageAndTruncate: (sessionId, messageId) => store.deleteMessageAndTruncate(sessionId, messageId),
      deleteMessage: (sessionId, messageId) => store.deleteMessage(sessionId, messageId),
    },
    clearPermissionSession: sessionId => Permission.clearSession(sessionId),
    getSkillsForSession,
    ...
    provider: {
      ...
      resolveApiKey: (providerId, providerConfig) => resolveProviderApiKey(providerId, providerConfig),
      resolveOAuthAuth: (providerId, apiKey) => authService.resolveProviderAuth(providerId, apiKey),
    ...
    models: {
      getModelContextLength: (model, providerId) => modelRegistry.getModelContextLength(model, providerId),
      getModelMaxOutputTokens: (model, providerId) => modelRegistry.getModelMaxOutputTokens(model, providerId),
    },
    buildHistoryMessages,
    buildResumeHistoryAfterToolConfirmation,
    executeMessageStream: executeMessageStream as unknown as MainStreamEngineRuntime['streams']['executeMessageStream'],
    executeAgentLoopStreamGeneration,
    compactSessionContext: options =>
      compactSessionContext(options as Parameters<typeof compactSessionContext>[0]),
    getContextCompactReason: options =>
      getContextCompactReason(options as Parameters<typeof getContextCompactReason>[0]),
    shouldSkipAutoCompactForProviderUsageMismatch: options =>
      shouldSkipAutoCompactForProviderUsageMismatch(
        options as Parameters<typeof shouldSkipAutoCompactForProviderUsageMismatch>[0],
      ),
