...
const imagePreviewUpdateHandlers = new Set<(payload: ImagePreviewUpdatePayload) => void>()
const TODO_PLAN_WEB_WINDOW_EVENT = 'todo-plan:web-window-action'
const sharedEventSources = new Map<string, {
  refCount: number
  source: EventSource
}>()
...
function dispatchTodoPlanWindowAction(
  ...
  const event = typeof CustomEvent === 'function'
    ...
    : ({ type: TODO_PLAN_WEB_WINDOW_EVENT, detail: payload } as unknown as Event)
...
function booleanProperty(value: unknown, key: keyof PlatformCapabilities, fallback: boolean): boolean {
  if (!value || typeof value !== 'object') return fallback
  const candidate = (value as Partial<Record<keyof PlatformCapabilities, unknown>>)[key]
...
async function refreshWebCapabilities(): Promise<PlatformCapabilities> {
  const capabilities = normalizeServerCapabilities(await requestJson('/api/capabilities'))
...
function createEventSourceSubscription<T>(
  ...
  const listener = (event: MessageEvent<string>) => {
    try {
      ...
      console.warn(`[Platform:web] Ignored malformed ${eventName} event`, error)
    }
  }
...
function createSessionMessagesChangedSubscription(
  callback: (data: { sessionId: string; action: 'added' | 'updated' | 'deleted'; messageId?: string }) => void,
): Unsubscribe {
  return createEventSourceSubscription<SessionEventEnvelope>('/api/events', 'session:event', (envelope) => {
    const event = envelope.event as { type?: string; message?: { id?: string }; messageId?: string } | undefined
    if (!event?.type) return

    if (event.type === 'message:user-created'
      || event.type === 'message:assistant-created'
      || event.type === 'message:created') {
      callback({ sessionId: envelope.sessionId, action: 'added', messageId: event.message?.id })
      return
    }

    if (event.type === 'message:updated' || event.type === 'messages:replaced') {
      callback({ sessionId: envelope.sessionId, action: 'updated', messageId: event.messageId })
      return
    }

    if (event.type === 'message:deleted') {
      callback({ sessionId: envelope.sessionId, action: 'deleted', messageId: event.messageId })
    }
  })
}

function createStepAddedSubscription(
  callback: (data: { sessionId: string; messageId: string; step: unknown }) => void,
): Unsubscribe {
  return createEventSourceSubscription<SessionEventEnvelope>('/api/events', 'session:event', (envelope) => {
    const event = envelope.event as { type?: string; step?: unknown } | undefined
...
function createStepUpdatedSubscription(
  callback: (data: { sessionId: string; messageId: string; stepId: string; updates: unknown }) => void,
): Unsubscribe {
  return createEventSourceSubscription<SessionEventEnvelope>('/api/events', 'session:event', (envelope) => {
    const event = envelope.event as { type?: string; stepId?: unknown; updates?: unknown } | undefined
    if (event?.type !== 'step:updated' || typeof event.stepId !== 'string') return
    callback({
      sessionId: envelope.sessionId,
      messageId: '',
      stepId: event.stepId,
...
function createSkillActivatedSubscription(
  callback: (data: { sessionId: string; messageId: string; skillName: string }) => void,
): Unsubscribe {
  return createEventSourceSubscription<SessionEventEnvelope>('/api/events', 'session:event', (envelope) => {
    const event = envelope.event as { type?: string; skillName?: unknown } | undefined
    if (event?.type !== 'skill:activated' || typeof event.skillName !== 'string') return
    callback({
      sessionId: envelope.sessionId,
      messageId: '',
...
function createContextSizeUpdatedSubscription(
  ...
  return createEventSourceSubscription<SessionEventEnvelope>('/api/events', 'session:event', (envelope) => {
    const event = envelope.event as { type?: string; contextSize?: unknown } | undefined
    if (event?.type !== 'context:size-updated' || typeof event.contextSize !== 'number') return
    callback({ sessionId: envelope.sessionId, contextSize: event.contextSize })
  })
}
...
function createContextCompactStartedSubscription(
  ...
  return createEventSourceSubscription<SessionEventEnvelope>('/api/events', 'session:event', (envelope) => {
    const event = envelope.event as { type?: string; message?: unknown } | undefined
...
function createContextCompactCompletedSubscription(
  callback: (data: { sessionId: string; success: boolean; error?: string }) => void,
): Unsubscribe {
  return createEventSourceSubscription<SessionEventEnvelope>('/api/events', 'session:event', (envelope) => {
    const event = envelope.event as { type?: string; success?: unknown; error?: unknown } | undefined
...
function isContextCompactStartedMessage(message: unknown): boolean {
  ...
  if (!content.includes('"context-compact"') || !content.includes('"compacting"')) return false
...
type WebUnsupportedPlatformMethod = typeof WEB_UNSUPPORTED_PLATFORM_METHODS[number]

const webUnsupportedApi = Object.fromEntries(
  WEB_UNSUPPORTED_PLATFORM_METHODS.map(method => [
    method,
    isSubscriptionMethod(method) ? unsupportedSubscription(method) : unsupported(method),
  ]),
...
const webApi = {
  ...
  searchExecuteAction: async (actionId: string) => {
    const response = await postJson<{ success: boolean; actionId?: string; error?: string }>('/api/search/actions', { actionId })
    if (response.success) emitSearchAction(response.actionId || actionId)
    return response
  },

  getThemes: () => requestJson('/api/themes'),
  getTheme: (themeId: string) => requestJson(`/api/themes/${encodeURIComponent(themeId)}`),
  applyTheme: (themeId: string, mode: 'dark' | 'light') =>
    postJson(`/api/themes/${encodeURIComponent(themeId)}/apply`, { mode }),
  refreshThemes: (projectPath?: string) => postJson('/api/themes/refresh', { projectPath }),
  openThemesFolder: () => postJson('/api/themes/open-folder'),

  getSystemPromptSnapshot: (sessionId: string) =>
    requestJson(`/api/sessions/${encodeURIComponent(sessionId)}/system-prompt-snapshot`),
  ...
  deletePrompt: (request: { id: string }) =>
    requestJson(`/api/prompts/${encodeURIComponent(request.id)}`, { method: 'DELETE' }),

  getSkills: (workingDirectory?: string) => {
    const query = workingDirectory ? `?workingDirectory=${encodeURIComponent(workingDirectory)}` : ''
  ...
  createSkill: (name: string, description: string, instructions: string, source: string) =>
    postJson('/api/skills', { name, description, instructions, source }),
  deleteSkill: (skillId: string) =>
    requestJson(`/api/skills/${encodeURIComponent(skillId)}`, { method: 'DELETE' }),
  ...
  enablePlugin: (pluginId: string) => postJson('/api/plugins/enable', { pluginId }),
  disablePlugin: (pluginId: string) => postJson('/api/plugins/disable', { pluginId }),
  ...
  onOAuthTokenRefreshed: (callback: (data: { providerId: string }) => void) =>
    createEventSourceSubscription<{ providerId: string }>('/api/oauth/events', 'oauth:token-refreshed', callback),
  onOAuthTokenExpired: (callback: (data: { providerId: string; error?: string }) => void) =>
    createEventSourceSubscription<{ providerId: string; error?: string }>('/api/oauth/events', 'oauth:token-expired', callback),
  ...
  gatewayWechatLogout: (request?: unknown) => postJson('/api/gateway/wechat/logout', request ?? {}),
  gatewayWechatAddAccount: (request?: unknown) => postJson('/api/gateway/wechat/accounts/add', request ?? {}),
  gatewayWechatStopAccount: (request: unknown) => postJson('/api/gateway/wechat/accounts/stop', request),
  gatewayWechatRemoveAccount: (request: unknown) => postJson('/api/gateway/wechat/accounts/remove', request),
  gatewayWechatRenameAccount: (request: unknown) => postJson('/api/gateway/wechat/accounts/rename', request),
  channelIdentityListLinks: (request?: unknown) => postJson('/api/channel-identity/links/list', request ?? {}),
  channelIdentityListProfiles: () => postJson('/api/channel-identity/profiles/list', {}),
  channelIdentityCreateProfile: (request: unknown) => postJson('/api/channel-identity/profiles/create', request),
  channelIdentityUpdateProfile: (request: unknown) => postJson('/api/channel-identity/profiles/update', request),
  channelIdentityCreateLink: (request: unknown) => postJson('/api/channel-identity/links/create', request),
  channelIdentityDeleteLink: (id: string) => postJson('/api/channel-identity/links/delete', { id }),
  channelIdentityResolve: (origin: unknown) => postJson('/api/channel-identity/resolve', { origin }),
  ...
  voiceSubmitUtterance: (request: unknown) => postJson('/api/voice/submit-utterance', request),
  voiceSubmitTranscript: (request: unknown) => postJson('/api/voice/submit-transcript', request),
  voiceSynthesize: (request: unknown) => postJson('/api/voice/synthesize', request),
  ...
  voiceGetTTSModels: (request?: unknown) => postJson('/api/voice/tts-models', request),
  ...
  voiceRuntimeEvent: (event: unknown) => postJson('/api/voice/runtime-event', event),
  onVoiceRuntimeCommand: (callback: (command: unknown) => void) =>
    createEventSourceSubscription('/api/voice/runtime-commands', 'voice:runtime-command', callback),
  acpGetAgents: () => requestJson('/api/acp/agents'),
  acpAddAgent: (config: unknown) => postJson('/api/acp/agents', { config }),
  acpUpdateAgent: (config: unknown) => postJson('/api/acp/agents/update', { config }),
  acpRemoveAgent: (agentId: string) => postJson('/api/acp/agents/remove', { agentId }),
  acpConnectAgent: (agentId: string) => postJson('/api/acp/agents/connect', { agentId }),
  acpDisconnectAgent: (agentId: string) => postJson('/api/acp/agents/disconnect', { agentId }),
  acpRefreshAgent: (agentId: string) => postJson('/api/acp/agents/refresh', { agentId }),
  ...
  createTodoPlanNote: (request: unknown) => postJson('/api/todo-plan/create', request),
  updateTodoPlan: (request: unknown) => postJson('/api/todo-plan/update', request),
  renameTodoPlanNote: (request: unknown) => postJson('/api/todo-plan/rename', request),
  deleteTodoPlanNote: (request: unknown) => postJson('/api/todo-plan/delete', request),
  ...
  deleteAgent: (agentId: string) =>
    requestJson(`/api/agents/${encodeURIComponent(agentId)}`, { method: 'DELETE' }),
  ...
  getModelsWithCapabilities: (providerId: string, options?: { forceRefresh?: boolean }) => {
    ...
    return requestJson(`/api/providers/${encodeURIComponent(providerId)}/models${query}`)
  },
  ...
  executeTool: (toolId: string, args: Record<string, unknown>, messageId: string, sessionId: string) =>
    postJson('/api/tools/execute', { toolId, arguments: args, messageId, sessionId }),
  cancelTool: (toolCallId: string) => postJson('/api/tools/cancel', { toolCallId }),
  updateToolCall: (sessionId: string, messageId: string, toolCallId: string, updates: unknown) =>
    postJson('/api/tools/update-call', { sessionId, messageId, toolCallId, updates }),
  ...
  mcpUpdateServer: (config: { id?: string }) =>
    postJson(`/api/mcp/servers/${encodeURIComponent(config.id || '')}/update`, config),
  mcpRemoveServer: (serverId: string) =>
    requestJson(`/api/mcp/servers/${encodeURIComponent(serverId)}`, { method: 'DELETE' }),
  ...
  watchWorkspace: (root: string) => postJson('/api/files/watch/start', { root }),
  unwatchWorkspace: (root: string) => postJson('/api/files/watch/stop', { root }),
  onWorkspaceFileChanged: (callback: (payload: { root: string; path: string; eventType: string }) => void) =>
    createEventSourceSubscription('/api/files/watch/events', 'workspace:file-changed', callback),
  ...
  listVariables: (sessionId: string) => postJson('/api/variables/list', { sessionId }),
  setVariable: (
    sessionId: string,
    name: string,
    value: string,
  ...
  ) => postJson('/api/variables/set', { sessionId, name, value, description, scope }),
  ...
  projectDirsRemove: (path: string) => postJson('/api/project-dirs/remove', { path }),
  ...
  readImageBase64: (filePath: string) => postJson('/api/media/read-image', { filePath }),
  listMediaAssets: (query?: { kind?: string; source?: string; search?: string; includeHidden?: boolean }) => {
  ...
  onImageGenerated: (callback: (payload: unknown) => void) =>
    createEventSourceSubscription('/api/media/events', 'media:image-generated', callback),
  openImagePreview: async (src: string, alt?: string) => {
    const response = await postJson<{ success?: boolean; previewId?: string }>('/api/media/preview/open', { src, alt })
    if (response.success) {
  ...
  readMemoryFile: (request: unknown) =>
    postJson('/api/memory/read', request),
  searchMemory: (request: unknown) =>
    postJson('/api/memory/search', request),
  appendMemory: (request: unknown) =>
    postJson('/api/memory/append', request),
  ...
  getMemoryLogStats: () =>
    postJson('/api/memory/logs/stats', {}),
  openMemoryLogFolder: () =>
    postJson('/api/memory/logs/open-folder', {}),
  cleanupMemoryLogs: () =>
    postJson('/api/memory/logs/cleanup', {}),
  ...
  getSessions: () => requestJson('/api/sessions?includeMessages=true'),
  createSession: (name: string) => postJson('/api/sessions', { name }),
  activateSession: (sessionId: string) => postJson(`/api/sessions/${encodeURIComponent(sessionId)}/activate`),
  getSession: (sessionId: string) => requestJson(`/api/sessions/${encodeURIComponent(sessionId)}`),
  switchSession: (sessionId: string) => postJson(`/api/sessions/${encodeURIComponent(sessionId)}/switch`),
  deleteSession: (sessionId: string) => requestJson(`/api/sessions/${encodeURIComponent(sessionId)}`, { method: 'DELETE' }),
  renameSession: (sessionId: string, newName: string) => postJson(`/api/sessions/${encodeURIComponent(sessionId)}/rename`, { name: newName }),
  ...
  updateSessionArchived: (sessionId: string, isArchived: boolean, archivedAt?: number | null) =>
    postJson(`/api/sessions/${encodeURIComponent(sessionId)}/archive`, { isArchived, archivedAt }),
  updateSessionWorkingDirectory: (sessionId: string, workingDirectory: string | null) =>
    postJson(`/api/sessions/${encodeURIComponent(sessionId)}/working-directory`, { workingDirectory }),
  updateSessionAgent: (sessionId: string, agentId: string) =>
    postJson(`/api/sessions/${encodeURIComponent(sessionId)}/agent`, { agentId }),
  updateSessionPermissionMode: (sessionId: string, permissionMode: unknown) =>
    postJson(`/api/sessions/${encodeURIComponent(sessionId)}/permission-mode`, { permissionMode }),
  updateSessionModel: (sessionId: string, provider: string, model: string) =>
    postJson(`/api/sessions/${encodeURIComponent(sessionId)}/model`, { provider, model }),
  updateSessionMaxTokens: (sessionId: string, maxTokens: number) =>
    postJson(`/api/sessions/${encodeURIComponent(sessionId)}/max-tokens`, { maxTokens }),
  getPendingPermissions: (sessionId: string) =>
    requestJson(`/api/sessions/${encodeURIComponent(sessionId)}/permissions/pending`),
  clearSessionPermissions: (sessionId: string) =>
    postJson(`/api/sessions/${encodeURIComponent(sessionId)}/permissions/clear`),
  listPermissionGrants: (options: unknown) => postJson('/api/permission-grants/list', options),
  revokePermissionGrant: (id: string) => postJson('/api/permission-grants/revoke', { id }),
  ...
  getSessionMessagesPage: (request: unknown) => postJson('/api/session-messages/page', request),
  ...
  getChatHistory: (sessionId: string) => postJson('/api/chat/history', { sessionId }),
  generateTitle: (message: string) => postJson('/api/chat/title', { message }),
  getSessionMessages: (sessionId: string) => postJson('/api/chat/messages', { sessionId }),
  getSessionTokenUsage: (sessionId: string) => postJson('/api/chat/token-usage', { sessionId }),
  ...
  removeFilesChangedMessage: (sessionId: string) =>
    postJson('/api/chat/remove-system-marker', { sessionId, markerType: 'files-changed' }),
  removeGitStatusMessage: (sessionId: string) =>
    postJson('/api/chat/remove-system-marker', { sessionId, markerType: 'git-status' }),
  ...
  updateMessageThinkingTime: (sessionId: string, messageId: string, thinkingTime: number) =>
    postJson('/api/chat/update-thinking-time', { sessionId, messageId, thinkingTime }),
  emitCommand: (sessionId: string, command: unknown) =>
    postJson(`/api/sessions/${encodeURIComponent(sessionId)}/commands`, command),
  ...
  abortStream: (sessionId?: string) => postJson('/api/streams/abort', { sessionId }),
  getActiveStreams: () => requestJson('/api/streams/active'),

  getSystemTheme: async () => ({ success: true, theme: getPreferredColorScheme() }),
  ...
  writeClipboardText: async (text: string) => {
    if (!navigator.clipboard?.writeText) {
      return { success: false, error: 'Clipboard write is not available in this browser.' }
    }
  ...
  openExternal: async (url: string) => {
    if (typeof window === 'undefined' || typeof window.open !== 'function') {
      return { success: false, error: 'Opening external URLs is not available in this browser.' }
    }
  ...
  onTodoPlanChanged: (callback: (payload: unknown) => void) =>
    createEventSourceSubscription('/api/todo-plan/events', 'todo-plan:changed', callback),

  listSchedulerTasks: () => requestJson('/api/scheduler/tasks'),
  getSchedulerTask: (request: unknown) => postJson('/api/scheduler/tasks/get', request),
  runSchedulerTaskNow: (request: unknown) => postJson('/api/scheduler/tasks/run-now', request),
  setSchedulerTaskEnabled: (request: unknown) => postJson('/api/scheduler/tasks/enabled', request),
  createSchedulerTask: (request: unknown) => postJson('/api/scheduler/tasks', request),
  updateSchedulerTask: (request: unknown) => postJson('/api/scheduler/tasks/update', request),
  deleteSchedulerTask: (request: unknown) => postJson('/api/scheduler/tasks/delete', request),
  listSchedulerRuns: (request: unknown) => postJson('/api/scheduler/runs', request),
  getSchedulerRun: (request: unknown) => postJson('/api/scheduler/runs/get', request),
