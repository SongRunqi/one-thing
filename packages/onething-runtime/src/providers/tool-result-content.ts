export type OnethingToolResultJsonValue =
  | string
  | number
  | boolean
  | null
  | OnethingToolResultJsonValue[]
  | { [key: string]: OnethingToolResultJsonValue | undefined }

export type OnethingToolResultRawValue =
  | OnethingToolResultJsonValue
  | object
  | undefined

type OnethingToolResultRawRecord = { [key: string]: OnethingToolResultRawValue }

type OnethingToolResultContentOutputPart =
...
export type OnethingToolResultOutput =
  | { type: 'json'; value: OnethingToolResultJsonValue }
  | { type: 'text'; value: string }
  | { type: 'content'; value: OnethingToolResultContentOutputPart[] }

export type OnethingCodexToolResultOutput =
  | string
  | Array<
...
function isRecord(value: OnethingToolResultRawValue): value is OnethingToolResultRawRecord {
  return Boolean(value && typeof value === 'object' && !Array.isArray(value))
}

export function onethingToolResultPayloadFromPart(
  part: OnethingToolResultRawValue,
): OnethingToolResultRawValue {
  if (!isRecord(part)) return part
  return part.result ?? part.output
}

function unwrapToolResultPayload(payload: OnethingToolResultRawValue): OnethingToolResultRawValue {
...
function mediaPartFromData(
  type: OnethingToolResultRawValue,
  data: OnethingToolResultRawValue,
  mediaType: OnethingToolResultRawValue,
  filename?: OnethingToolResultRawValue,
): OnethingToolResultContentOutputPart | null {
...
function addToolResultContentParts(
  payload: OnethingToolResultRawValue,
  parts: OnethingToolResultContentOutputPart[],
): boolean {
  const value = unwrapToolResultPayload(payload)
  if (!isRecord(value)) return false

  let hasMedia = false
  const addText = (text: OnethingToolResultRawValue) => {
...
function sanitizeJsonValue(value: OnethingToolResultRawValue): OnethingToolResultJsonValue {
  try {
    return JSON.parse(JSON.stringify(value ?? null, (_key, nestedValue: OnethingToolResultRawValue) => {
      ...
      return nestedValue
    })) as OnethingToolResultJsonValue
  } catch {
    return String(value)
  }
}

export function onethingToolResultToText(payload: OnethingToolResultRawValue): string {
  const value = unwrapToolResultPayload(payload)
...
export function onethingToolResultToAISDKOutput(
  payload: OnethingToolResultRawValue,
): OnethingToolResultOutput {
  const parts: OnethingToolResultContentOutputPart[] = []
...
export function onethingToolResultToCodexOutput(
  payload: OnethingToolResultRawValue,
): OnethingCodexToolResultOutput {
  const parts: OnethingToolResultContentOutputPart[] = []
  if (!addToolResultContentParts(payload, parts)) {
    return onethingToolResultToText(payload)
  }

  const output: Exclude<OnethingCodexToolResultOutput, string> = []
