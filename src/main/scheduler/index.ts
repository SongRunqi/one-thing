...
function schedulerStatePath(): string {
  return path.join(getStorePath(), 'scheduler', 'state.json')
}
...
export class Scheduler {
    ...
    try {
      ...
      if (enabled && schedule) {
        nextRunAt = changed || !state.nextRunAt
          ? this.computeInitialNextRunAt(schedule, state, nowMs())
          : state.nextRunAt
      }
  ...
  private ensureLoaded(): void {
    ...
    this.loaded = true
    try {
      const filePath = schedulerStatePath()
      ...
      console.error('[Scheduler] Failed to load scheduler state:', error)
  ...
  private save(): void {
    ...
    try {
      ...
      console.error('[Scheduler] Failed to save scheduler state:', error)
    }
  }
  ...
  private computeFollowingNextRunAt(schedule: SchedulerSchedule, scheduledFor: number): number | undefined {
    const after = Math.max(nowMs(), scheduledFor)
    if (schedule.kind === 'cron') {
