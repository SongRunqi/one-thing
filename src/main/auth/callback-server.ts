...
interface CallbackRegistration {
  ...
  path: string
  expiresAt: number
  onCallback: (params: { code: string; state: string; flowId: string; providerId: string }) => Promise<void>
}
...
export class CallbackServerManager {
  ...
  async registerFlow(options: {
    ...
    ports: number[]
    timeoutMs: number
    onCallback: CallbackRegistration['onCallback']
  }): Promise<{ redirectUri: string; port: number }> {
