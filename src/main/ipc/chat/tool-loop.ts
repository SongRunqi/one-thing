...
import { formatMessagesForLog, buildSystemPrompt, buildHistoryMessages, getTextFromContent, type HistoryMessage } from './message-helpers.js'
...
export async function executeStreamGeneration(
  ...
  try {
    ...
    // Get session info (needed for tool init context)
    const session = store.getSession(ctx.sessionId)
    ...
    // This provides available skills, working directory, and provider config
