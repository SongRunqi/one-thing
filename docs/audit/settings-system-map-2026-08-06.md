...
> **同日后置更正**：本快照拍摄后，soul-memory 插件于当天整树退役
> （`docs/audit/soul-memory-retirement-2026-08-06.md`）。因此下文中
> `general.soulMemory` 嵌套段与 `app/plugins/builtin/soul-memory.ts` 的写入点
> **均已不存在**；`general` 的键数与体积相应减少。快照其余部分不受影响，
> 数字保持拍摄当时原样，未回改。
