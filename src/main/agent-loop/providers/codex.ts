...
import { agentToolMessageContentToStructuredPayload } from '../tool-results.js'
import type {
  AgentContentPart,
  AgentFinishReason,
  AgentJsonObject,
...
async function resolveCodexTokenForRequest(
  ...
  if (hasOAuthCredentials) {
    try {
      ...
      return forceRefresh
        ? await authService.refreshToken(CODEX_PROVIDER_ID)
        : await authService.refreshTokenIfNeeded(CODEX_PROVIDER_ID)
    } catch (error) {
