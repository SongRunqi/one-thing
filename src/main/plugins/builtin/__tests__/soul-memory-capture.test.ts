...
function makeWorkspace(
	...
	return {
		settings: normalizeSoulMemorySettings(
			soulMemory as any,
		) as ResolvedSoulMemorySettings,
		agentId: "default",
		root,
		memoryDir: path.join(root, "daily"),
		...
		todayPath: path.join(root, "daily", "2026-06-16.md"),
...
describe("soul memory capture routing", () => {
	it("uses action JSON for capture and markdown bullets for daily-note extraction", () => {
		...
		expect(__testing.dailyNoteExtractionSystemPrompt).toContain(
			"Do not emit any candidate intended for short-term.jsonl",
		);
		expect(__testing.dailyNoteExtractionSystemPrompt).toContain(
			"Return markdown bullets only",
		);
		expect(__testing.dailyNoteExtractionSystemPrompt).toContain(
			'"给我讲讲 Go 的 array" becomes "用户今天学习了 Go array。"',
		);
	...
	it("allows 10-turn review prompts to update SOUL.md and DREAMS.md", () => {
		expect(__testing.memoryReviewSystemPrompt).toContain(
			"every fixed number of user turns",
		);
		expect(__testing.memoryReviewSystemPrompt).toContain(
			"SOUL.md, DREAMS.md, USER.md, and MEMORY.md",
		);
		expect(__testing.memoryReviewSystemPrompt).toContain(
			'"target":"soul|dreams|user|memory"',
		);
		expect(__testing.memoryReviewSystemPrompt).toContain(
			"promoted into SOUL.md",
		);
	});
	...
	it("treats NONE or JSON as no daily-note bullets", () => {
		...
		expect(
			__testing.parseDailyNoteBullets(
				'{"action":"none","explicit":false,"confidence":0.99,"candidates":[]}',
			),
		).toEqual([]);
	});
	...
	it("rejects raw user request echoes from daily capture", () => {
		expect(
			__testing.parseDailyNoteBullets(
				"- sdk怎么添加已经安装的java jdk？ idea里面安装的，brew安装的，等等？",
			),
		).toEqual([]);
	});
	...
	it("keeps transformed daily learning notes instead of raw question echoes", () => {
		expect(
			__testing.parseDailyNoteBullets("- 用户今天学习了 Go array。"),
		).toEqual(["用户今天学习了 Go array。"]);
	});
	...
	it("keeps real user preferences while filtering request echoes", () => {
		expect(
			__testing.parseDailyNoteBullets("- 用户说：我讨厌模棱两可的说法"),
		).toEqual(["用户说：我讨厌模棱两可的说法"]);
	});
	...
	it("parses add, replace, and remove capture actions from compact JSON", () => {
		const parsed = __testing.parseDailyNoteCaptureResult(
			JSON.stringify({
				action: "capture",
				confidence: 0.88,
				memories: [
					...
					},
					{
						...
						newText:
							"用户今天确认 capture 应支持 add/replace/remove 三种 daily-note 动作。",
					},
					{
						action: "remove",
	...
	it("appends extracted capture content to the daily note without writing short-term signals", async () => {
		const root = await fsp.mkdtemp(
			path.join(os.tmpdir(), "soul-memory-daily-test-"),
		);
		tempDirs.push(root);
		const settings = createDefaultSettings();
		...
		const target = await __testing.appendDailyNoteBullets({
			settings,
			bullets: [
				"用户今天在 aikefu-bridge 排查 mvn compile 的 cannot find symbol 问题。",
			],
			heading: "10:30:00",
		});

		expect(target?.relativePath).toMatch(/^daily\/\d{4}-\d{2}-\d{2}\.md$/);
		...
		await expect(
			fsp.stat(path.join(root, "daily", ".dreams", "short-term.jsonl")),
		).rejects.toMatchObject({ code: "ENOENT" });
	});

	it("applies capture add, replace, and remove actions to the daily note", async () => {
		const root = await fsp.mkdtemp(
			path.join(os.tmpdir(), "soul-memory-daily-actions-test-"),
		);
		tempDirs.push(root);
		const settings = createDefaultSettings();
		settings.general.soulMemory = {
		...
		const dailyPath = path.join(root, "daily", `${today}.md`);
		...
		const result = await __testing.applyDailyNoteCaptureActions({
			...
			candidates: [
				...
				},
				{
					...
					confidence: 0.9,
					content:
						"用户今天在 start-electron 实现 capture JSON action mutation。",
				},
			],
			heading: "10:30:00",
		});
	...
	it("builds review input with SOUL.md and DREAMS.md context", async () => {
		const root = await fsp.mkdtemp(
			path.join(os.tmpdir(), "soul-memory-review-input-test-"),
		);
		tempDirs.push(root);
	...
	it("applies review add, replace, and remove actions to SOUL.md and DREAMS.md", async () => {
		const root = await fsp.mkdtemp(
			path.join(os.tmpdir(), "soul-memory-review-files-test-"),
		);
		tempDirs.push(root);
		const workspace = makeWorkspace({}, root);
		await fsp.mkdir(root, { recursive: true });
		await fsp.writeFile(
			workspace.soulPath,
			["# SOUL.md", "", "Keep replies formal.", "Remove stale voice.", ""].join(
				"\n",
			),
			"utf-8",
		);
		...
		const results = [];
		results.push(
			await __testing.applyMemoryReviewCandidate(
				workspace,
				{
					action: "replace",
					target: "soul",
		...
		expect(dreams).toContain(
			"Watch for repeated requests about concrete examples.",
		);
