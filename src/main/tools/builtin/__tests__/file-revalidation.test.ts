...
describe('file tool policy revalidation', () => {
  ...
  it('refuses edit when file changed after policy approval and approved diff changes', async () => {
    ...
    await expect(EditTool.execute({
      path: filePath,
      edits: [{ oldText: 'A: old', newText: 'A: new' }],
    }, { ...ctx, approvedAnalysis: analysis })).rejects.toThrow('File changed after permission approval')

    await expect(fs.readFile(filePath, 'utf-8')).resolves.toBe('A: old\nB: external\n')
  })
  ...
  it('fails clearly when an approved edit no longer applies to latest content', async () => {
    ...
    await expect(EditTool.execute({
      path: filePath,
      edits: [{ oldText: 'A: old', newText: 'A: new' }],
    }, { ...ctx, approvedAnalysis: analysis })).rejects.toThrow('Failed to edit')
