...
| `plugins/` | 插件系统：日志监控插件、"note-skills"（Obsidian vault 技能发现）、插件命令执行/列举，以及体量很大的 `soul-memory.ts`（140k，记忆捕获/回顾候选提取逻辑）。 |
...
| `editor/` | Monaco/ProseMirror 编辑栈：`MonacoEditor.vue`（代码编辑）、`MarkdownDocumentEditor.vue`（markdown 撰写）、`prose/` 子目录（ProseMirror 富文本笔记编辑器：schema/commands/editor）、`triggers.ts`（`/`、`@` 触发器）。 |
