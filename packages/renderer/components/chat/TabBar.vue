...
.topbar-sidebar-actions-slot {
  ...
  display: flex;
  align-items: center;
  /* 空盒子在 align-items:center 的父级里会塌成 0 高,而 app-region 只算
     content box —— 零面积的 no-drag 挖不掉 .tab-bar 的 drag,浮层按钮
     (跨分支 fixed,自身 no-drag 无效)就会被拖拽区吃掉点击。必须撑满行高。 */
  align-self: stretch;
  flex-shrink: 0;
  overflow: hidden;
  padding-left: 0;
...
/* 宽度对齐 App.vue 里 SidebarActionGroup 的右缘:
   ...
   padding 不算进 content box,所以 padding-left 必须等于按钮左缘 84px,
   no-drag 的 content box 才恰好盖住 [84,160] 的按钮区。 */
