...
function createOpenAICompatibleApiError(
  ...
  status: number,
  responseBody: string,
): Error & { responseBody: string; data: { providerId: string; statusCode: number; responseBody: string } } {
...
export function createOpenAICompatibleAgentProvider(options: OpenAICompatibleAgentProviderOptions): AgentProvider {
  ...
  async function* streamTurn(request: AgentTurnRequest): AsyncGenerator<AgentTurnStreamEvent, void, void> {
    ...
    if (!response.ok) {
      ...
      throw createOpenAICompatibleApiError(options.providerId, response.status, text)
    }
