...
#### 3.3.5.1 L2.5 续:用户自己的壁纸(2026-08-10 落地,B 期)

§3.3.5 第 4 条"不做"里没有"用户换图"这一项,但它也不在做的范围里 ——
`image` 是包内相对路径,换图 = 发版本。真实需求补上了这一格:**用户想用
自己的图**。三件事,全都是"往窄腰上加一块枚举能力",红线一条没动。

1. **`file-pick` 描述树节点**(词表 append-only 加一型)。核心裁决:
   **字节不过插件的手**。插件声明 `{ type: 'file-pick', label, accept?,
   maxBytes?, actionId }`,宿主画按钮 → 宿主拉原生 `showOpenDialog` →
   宿主在拷贝入口过扩展名 + 尺寸闸 → 宿主拷进
   `plugins/<id>/storage/imports/<清洗后的名字>` → `onAction` 收到
   `{ path: 'storage:imports/<name>', name, size }`。插件拿不到用户磁盘上的
   路径,更拿不到文件内容。
   - `accept` **只能收窄**(宿主背景扩展名白名单的子集),超出即整棵树拒;
   - `maxBytes` 是唯一被**钳**的旋钮(硬顶 10MB,声明更大按 10MB 算);
   - 取消 = **无 action**;闸不过 = 宿主 toast + 无 action + **不计熔断**
     (用户选错文件不是插件的失败)。
   - **手势锚定是天然的**:原生对话框只能由用户那一次点击拉起来,这条路径上
     没有(也不需要)可伪造的 `userGesture` 布尔。
2. **`storage:` 寻址**。运行期图源多一格值域:`storage:<rel>` →
   `plugins/<id>/storage/<rel>`。路径判据复用包内资产那一份(穿越/绝对/
   编码变体全拒)+ 图片扩展名白名单,只在"相对哪个根"上分叉。
   URL 走**同 scheme、同 origin 的保留首段** `__storage__`:
   `onething-plugin://<id>/__storage__/<rel>`。
   - **为什么不开第二个 scheme / 第二个 host**:那会把 CSP 与 origin 的同步点
     从一处变成三处(第二次 registerSchemesAsPrivileged、第二个 CSP builder、
     每条 CSP 里多一个 host-source),而隔离并不因此变强。保留首段让协议
     handler 只多一条分支:首段是 `__storage__` 就只查数据根,否则只查包根,
     两条各自 realpath 复核,**谁也够不到谁**。代价是 `__storage__` 这个名字
     在包里被保留了(双下划线哨兵)。
   - **代码区与数据区不得互相越界**是这一格的硬约束,靠"选根"这一步执行,
     不靠事后比对两个前缀。数据区额外只服务图片(比 MIME 白名单严):
     那目录里还躺着插件自己写的 storage JSON。
3. **`api.theme.updateBackground` 放开 `image`,仅收 `storage:` 前缀**。
   包内换图 = 发版本这条语义**一个字没变**;manifest 缺省图仍是启动兜底。
   非法寻址或文件不存在 → **拒整条调用**(不是丢一个字段:半条命令比不执行
   更难解释),背景保持原样,不计熔断。运行期图压过缺省图并接管 darkImage ——
   用户挑的是"这一张",不是"浅色这一张"。
   **仍不持久**(far-hills 模式):插件自己记住,启动时重放。

`file-pick` 归在决策表的"表单 → L1"那一格:它是一个受宿主托管的输入控件,
不是一条新的能力泳道。

#### 3.3.5.2 勘误(2026-08-10):选文件不止一个家 —— 设置 schema 的 `file-import`

B 期落地时,"选文件"只有 `file-pick` 节点这一个入口,而描述树的家是**面板**。
于是壁纸插件把"选一张图"做成了工作台面板 —— 用户的批评一针见血:**这是配置,
配置的家是设置页;面板该留给活内容**。

根因不在插件作者,在宿主:R3 的设置 schema 子集(boolean / string / number /
integer / enum / string 数组)**没有文件控件**,想让用户选个文件就只剩描述树
这一条路,而描述树的渲染位置把 UX 一路拽进了面板。**能力缺口把 UX 拽错了位置**。

补法是往同一个窄腰上再加一块枚举能力,不是加一条泳道:
`contributes.settings.schema` 的属性支持 `{"type":"string",
"format":"file-import","accept":[…],"maxBytes":…}`,宿主在设置页画"选择文件"
按钮 + 当前值,点击走**同一条**托管导入链(原生对话框 → 宿主校验 → 宿主拷进
`plugins/<id>/storage/imports/` → 回一个 `storage:` 地址),存进该配置字段。

- 落在 `format` 而不是自造键:`format` 是 JSON Schema 自己的扩展位,不认识它的
  读者按规范**忽略**它,于是这条声明在别处退化成普通 string 字段,而不是
  "非法 schema"。宿主对**未知 format** 同样是忽略。
- `accept` / `maxBytes` 的语义与 `file-pick` 节点**逐字相同**,判据也是**同一份**
  (`core/plugins/file-pick.ts` 的 `describePluginFileImportDeclarationProblem`)——
  两处各写一份,今天相同,明天白名单变了就只改一边,得到的是"面板里能导入、
  设置页说不支持"这种没人能解释的分裂。
- 非法声明按 R3 既有惯例:整块配置区显示"schema 不受支持"并逐条列出原因,
  **插件照常加载**(不拒载、不熔断)。

**放置判例(通用,不只是文件):配置类交互归设置页,面板留活内容。**
一个交互属于哪一边,看它改的是"设置一次就长期生效的偏好"(→ 设置页),
还是"此刻正在发生的东西"(→ 面板 / 锚点块)。宿主缺控件时**补控件**,
不要把配置搬进面板将就。
...
| 让用户**配置**一个文件(选一次,长期生效) | 设置 schema `format: 'file-import'`(§3.3.5.2) | 壁纸插件换图 |
| 让用户在面板里**现场**导入一个文件 | L1 `file-pick`(B 期,§3.3.5.1) | 批量导入面板 |
