...
describe("same-file edit ordering", () => {
	...
	it("preserves multiple concurrent edit tool calls against one file under TMP", async () => {
			...
			try {
				...
				return await EditTool.execute(
					...
					},
					{
						...
						metadata: vi.fn((update) =>
							metadataByCall.get(callId)?.push(update),
						),
		...
		await expect(fs.readFile(filePath, "utf-8")).resolves.toBe(
			["A: new", "B: new", "C: new", ""].join("\n"),
		);
		// Each edit is based on the previous edit's output: the content hashes chain.
		expect(first.metadata.afterContentHash).toBe(
			second.metadata.originalContentHash,
		);
		expect(second.metadata.afterContentHash).toBe(
			third.metadata.originalContentHash,
		);
