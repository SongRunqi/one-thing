...
function buildEditPlan(
  ...
  const diff = createTwoFilesPatch(resolvedPath, resolvedPath, preview.baseContent, preview.newContent)
...
export function createEditTool(adapters: EditToolAdapters): Tool.Info<typeof EditParameters, EditMetadata> {
  return Tool.define<typeof EditParameters, EditMetadata>('edit', {
    ...
    async analyze(args, ctx) {
      ...
      const resolvedPath = resolveEditPath(args.path, ctx.workingDirectory, adapters)
    ...
    async execute(args, ctx) {
      ...
      const resolvedPath = resolveEditPath(inputPath, ctx.workingDirectory, adapters)
