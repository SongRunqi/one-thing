...
走查条目答的是「按剧本走一遍对不对」;这一节记的是**走的过程中撞见的缺陷** ——
现象 / 根因 / 修复语义 / 怎么复验各一段。编号 F 系列,与 V 系列平行。

### F1 · 幽灵排队:裁决答空之后手不放,状态条永远「N 人排队中」

**现象** 群房 4 人举手 → 裁判批量裁决**答空**(「报数游戏,这轮无人发言」)→ 四只手
原地留队,UI 的状态条从此写着「4 人排队中」再不清零;`blockedBy` 还标成**等座位**,
而座位 0/6 全空。`onething collab inspect` 佐证:`举手 4(等座位) / 持牌 0 / 裁决 空`。

**根因** `collab/actors/floor-policy.ts` 的 `free` 策略在 `resolved` 分支上有一个**刻意
决定**:「没被排上的手继续留在队里等下一次触发 —— 丢掉它们等于把『这轮你先别说』
读成『你以后也别说了』」。这个前提在 D6-a 之后不成立了:举手判据那时改成了「每条
房间事实**人人机械举手**」(`runtime.ts` 的 `wiredHandEvaluator`),下一条消息全员都会
重新举一次手。留旧手因此防不住任何东西,只造出两样坏东西 —— 永不清零的排队数,
以及**旧话题的手掺进新话题的裁决窗**(金重放里看得最清楚:ana 为 rf-2 举的手,跟着
bo 那扇为 rf-3 开的窗一起被判、一起拿牌)。

**修复语义** 裁决是对**这一批候选**的终审,手随消息走:

1. **判过、没被点名 → 当场放下**。空裁决 = 全部放下(这一轮的沉默是合法答案);
2. **点名了但座位不够 → 保留**。D3 的分批兑现照旧(verdict `["bo","ana"]` + 单座位:
   bo 拿牌,ana 留队等 bo 让位,不重开窗、不再买一次调用);
3. **不在候选集里 → 保留**。窗在飞的时候才举的手一次都没被判过,连坐放下等于替裁判
   否掉一个它没看过的人 —— 它们进下一扇窗;
4. **降级(`degraded`)路径不变**:那不是一个答案,房间回落举手 FIFO,手被消费成授牌。

判据落点:「这一次判的是谁」由**裁判报回来**(`verdictCandidates`),不由房间猜 ——
窗是第一只手开的,其余的手随后异步到,只认开窗那一刻的快照的话,四只手的房只放得下
一只。房账 `judgment.candidates` 记同一件事:开窗时以队列现状打底,裁决回来时被裁判
那份覆盖。放手发生在**账转换层**(`room-rules.ts` 的队列结算),不在 `decide()` 里 ——
`decide` 是纯决策,手的增删是账的事。放手还是**一次性**的:分批兑现时留在窗里的那半份
裁决把候选集清空,否则同一个人为下一条消息重新举的手会被这份旧答案再放一次(一只刚
举起来的手凭空消失,比幽灵排队更难查)。

**顺带修的 `blockedBy`** 挂了裁判的 `free` 房里,闸全开着还举着手,意思是**等下一扇窗**
而不是等座位。判据从兜底的 `seats` 改成 `judging`(「裁决中」),不再当着一排空座位说
「等人让位」。

**存量僵尸手不需要迁移**:旧账的窗上没有候选集那一格,按「这一批就是全部」处理 ——
下一份裁决落地即全部放下;更常见的是下一条消息开新窗把它们裁一遍(手在队里就是候选),
未点名即放下。两条路都自愈。

**怎么复验** 造一间 4 人群房,发一条**不需要谁回应**的消息(比如「大家先看着,不用回」),
让裁判答空:

- 状态条的排队数**清零**(不是 4,也不是慢慢降到 0);
- `onething collab inspect --room <名字>`:`举手 0`,若仍有手,判据应是 `裁决中` 而不是
  `等座位`;
- 紧接着再发一条真的要人答的消息,能正常排上 —— 放手没有把房间放哑。

**落点** `collab/actors/room-rules.ts`(队列结算 + `resolveCollabRoomHandBlock`)、
`collab/actors/referee-rules.ts`(`candidates` / `verdictCandidates`)、
`app/collab/actors/referee-actor.ts`(报回现取的候选集)。金重放
`referee-free.room-actor.expected.txt` 随语义更新;`ring` / `waves` / `phase` 三档零变化
(它们不走这条分支)。
