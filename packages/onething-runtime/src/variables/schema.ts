...
function parseGlobalVariable(value: unknown): VariablesFileGlobalVariable | null {
...
export function parseVariablesFile(raw: unknown): {
  ...
  const globalsRaw = Array.isArray(raw.global_variables) ? raw.global_variables : []
  const globalVariables = globalsRaw.map(parseGlobalVariable)
  if (globalVariables.some(value => !value)) {
    console.warn('[variables.store] persisted file failed schema validation, falling back to defaults')
    return { data: createDefaultVariablesFile(), recovered: true }
  }

  const data: VariablesFile = {
    ai_note_dir: typeof raw.ai_note_dir === 'string' ? raw.ai_note_dir : '~/.onething/notes',
    user_note_dir: typeof raw.user_note_dir === 'string' ? raw.user_note_dir : '',
    work_note_dir: typeof raw.work_note_dir === 'string' ? raw.work_note_dir : '',
