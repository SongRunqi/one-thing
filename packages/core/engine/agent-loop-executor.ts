import { toJsonObject, toJsonValue, type JsonObject, type JsonValue } from '../json.js'
...
import { getTextFromContent, type CoreAIMessageContent } from './message-content.js'

export interface CoreAgentLoopExecutorContentAccumulator {
  value: string
}

export interface CoreAgentLoopExecutorTurnState<TToolCall = unknown, TContentPart = unknown> {
  toolCalls: TToolCall[]
...
export interface ApplyAgentLoopProviderDataWithAdaptersOptions<TContentPart extends CoreOrderedPartLike> {
...
export interface ApplyAgentLoopProviderDataRuntimeOptions<TContentPart extends CoreOrderedPartLike>
...
> {
  state: CoreAgentLoopFinishState<CoreAgentLoopExecutorTurnState<TToolCall, TContentPart>> & {
  ...
  emitter: CoreAgentLoopContentPartEmitter<TContentPart> &
    CoreAgentLoopToolExecutionEmitter<TToolCall, TStepUpdate, CoreToolPartialResultUpdate, TToolResult> & {
  ...
  createTurnState: () => CoreAgentLoopExecutorTurnState<TToolCall, TContentPart>
  syncAccumulatedUsage?: (usage: CoreAgentLoopUsage) => void
  syncLastTurnUsage?: (usage: CoreAgentLoopUsage) => void
  updateStepsUsageByTurn?: (turnIndex: number, usage: CoreAgentLoopUsage) => void
  now?: () => number
}
...
> {
  ...
  streamChunks(prepared: TSupportedPrepared): AsyncIterable<AgentProviderStreamChunk>
  ...
  emitFinalAssistantMessageUpdate(): CoreMaybePromise<void>
  sendStreamError(data: { error: string; preserved: boolean }): CoreMaybePromise<void>
  sendStreamComplete(data: { sessionName?: string; error?: string }): CoreMaybePromise<void>
...
export interface ApplyAgentLoopTextChunkWithAdaptersOptions<TContentPart extends CoreOrderedPartLike> {
  text: string
  turnIndex: number
  content: CoreAgentLoopExecutorContentAccumulator
  orderedParts: TContentPart[]
...
export interface ApplyAgentLoopReasoningChunkWithAdaptersOptions<TContentPart extends CoreOrderedPartLike> {
  reasoning: string
  turnIndex: number
  accumulatedContent: string
  turn: Pick<CoreAgentLoopExecutorTurnState<unknown, TContentPart>, 'orderedParts' | 'content' | 'reasoning' | 'toolCalls' | 'hasSentToolParts'>
  handleReasoningChunk(
    reasoning: string,
    accumulator: CoreAgentLoopExecutorContentAccumulator,
    turnIndex: number,
...
export interface CoreAgentLoopToolContentPartDispatchPlan<TPart extends CoreOrderedPartLike> {
  shouldSend: boolean
  parts: TPart[]
  dataStepsPart?: CoreAgentLoopDataStepsPart
}

export interface CoreAgentLoopTurnContentPersistencePlan<TPart extends CoreOrderedPartLike> {
...
export interface CoreAgentLoopContentPartStore<TPart> {
  addMessageContentPart(sessionId: string, assistantMessageId: string, part: TPart | CoreAgentLoopDataStepsPart): void
}
...
export interface DispatchAgentLoopToolContentPartsOptions<TPart extends CoreOrderedPartLike> {
  turn: Pick<CoreAgentLoopExecutorTurnState<unknown, TPart>, 'orderedParts' | 'hasSentToolParts'>
  turnIndex: number
  emitter: CoreAgentLoopContentPartEmitter<TPart>
}

export interface PersistAgentLoopTurnContentPartsOptions<TPart extends CoreOrderedPartLike> {
  sessionId: string
  assistantMessageId: string
  turn: Pick<CoreAgentLoopExecutorTurnState<unknown, TPart>, 'orderedParts' | 'toolCalls'>
  turnIndex: number
...
export interface CoreAgentLoopToolResultStepUpdate<TToolCall, TToolResult = CoreToolResult> {
...
export interface CoreAgentLoopToolExecutionEndUpdate<TToolResult = CoreToolResult> {
  result?: TToolResult
  isError: boolean
  error?: string
}

export interface CoreAgentLoopToolResultPresentation<TToolCall, TToolResult = CoreToolResult> {
...
export interface CoreAgentLoopToolInputProcessor<TToolCall extends CoreAgentLoopToolCallForSettlement> {
  toolCalls: TToolCall[]
  ...
  handleToolCallChunk(chunk: { toolCallId: string; toolName: string; args: JsonObject }): TToolCall
}
...
export interface ApplyAgentLoopToolInputStartWithAdaptersOptions<
  ...
  TStepUpdate,
> {
  turn: Pick<CoreAgentLoopExecutorTurnState<TToolCall, TContentPart>, 'orderedParts' | 'hasSentToolParts'>
  turnIndex: number
  toolCallId: string
  toolName: string
  processor: Pick<CoreAgentLoopToolInputProcessor<TToolCall>, 'handleToolInputStart' | 'getStepIdForToolCall'>
  stepIdsByToolCallId: Map<string, string>
  emitter: CoreAgentLoopContentPartEmitter<TContentPart> & Pick<CoreAgentLoopToolExecutionEmitter<TToolCall, TStepUpdate>, never>
}
...
export interface ApplyAgentLoopToolInputEndWithAdaptersOptions<
  ...
  TToolCall extends CoreAgentLoopToolCallForSettlement,
  TStepUpdate,
> extends Omit<StartAgentLoopToolExecutionOptions<TToolCall, TStepUpdate>, 'toolCall' | 'toolCalls' | 'stepId'> {
  toolCallId: string
  turn: Pick<CoreAgentLoopExecutorTurnState<TToolCall, TContentPart>, 'toolCalls'>
  processor: Pick<CoreAgentLoopToolInputProcessor<TToolCall>, 'handleToolInputEnd' | 'getStepIdForToolCall' | 'toolCalls'>
  stepIdsByToolCallId: Map<string, string>
}
...
export interface ApplyAgentLoopToolCallFallbackWithAdaptersOptions<
  ...
  TStepUpdate,
> extends Omit<StartAgentLoopToolExecutionOptions<TToolCall, TStepUpdate>, 'toolCall' | 'toolCalls' | 'stepId'> {
  turn: Pick<CoreAgentLoopExecutorTurnState<TToolCall, TContentPart>, 'orderedParts' | 'hasSentToolParts' | 'toolCalls'>
  turnIndex: number
  toolCall: CoreAgentLoopFallbackToolCallLike
  processor: CoreAgentLoopToolInputProcessor<TToolCall>
  stepIdsByToolCallId: Map<string, string>
  emitter: CoreAgentLoopContentPartEmitter<TContentPart> & StartAgentLoopToolExecutionOptions<TToolCall, TStepUpdate>['emitter']
}
...
export interface ApplyAgentLoopToolResultWithAdaptersOptions<
  ...
  TStepUpdate,
  TToolResult = CoreToolResult,
> extends SettleAgentLoopToolResultOptions<TToolCall, TStepUpdate, TToolResult> {
  state: {
    toolIterations: number
    skillManageCalled: boolean
  }
...
> {
  ...
  stepId?: string
  store: CoreAgentLoopToolExecutionStore<TToolCall>
  emitter: Pick<CoreAgentLoopToolExecutionEmitter<TToolCall, TStepUpdate>, 'sendToolCall' | 'sendToolExecutionStart' | 'sendStepUpdated'>
  now?: () => number
}
...
> {
  ...
  stepIdsByToolCallId: Map<string, string>
  store: CoreAgentLoopToolExecutionStore<TToolCall>
  emitter: Pick<CoreAgentLoopToolExecutionEmitter<TToolCall, TStepUpdate, CoreToolPartialResultUpdate, TToolResult>, 'sendToolResult' | 'sendToolExecutionEnd' | 'sendStepUpdated'>
  now?: () => number
}
...
> {
  ...
  toolCalls: TToolCall[]
  stepIdsByToolCallId: Map<string, string>
  emitter: Pick<CoreAgentLoopToolExecutionEmitter<TToolCall, TStepUpdate>, 'sendStepUpdated'>
}

export interface ApplyAgentLoopToolPartialResultOptions<
  TToolCall,
  TStepUpdate,
  TPartialResult extends CoreToolPartialResultUpdate = CoreToolPartialResultUpdate,
> {
  toolCallId: string
  update: TPartialResult
  stepIdsByToolCallId: Map<string, string>
  emitter: Pick<CoreAgentLoopToolExecutionEmitter<TToolCall, TStepUpdate, TPartialResult>, 'sendToolExecutionUpdate' | 'sendStepUpdated'>
}
...
export interface CoreAgentLoopAfterAssistantResponseContext<
  ...
  TProviderConfig,
  TSettings,
> extends CoreAgentLoopPostResponseTriggerContext<TSession, TMessage, TProviderConfig, TSettings> {
  assistantMessageId: string
}
...
export interface CoreAgentLoopPostResponseContexts<
  ...
  TSettings,
> {
  triggerContext: CoreAgentLoopPostResponseTriggerContext<TSession, TMessage, TProviderConfig, TSettings>
  afterAssistantResponseContext: CoreAgentLoopAfterAssistantResponseContext<TSession, TMessage, TProviderConfig, TSettings>
}
...
> {
  ...
  runTriggerContext: (
    context: CoreAgentLoopPostResponseTriggerContext<TSession, TMessage, TProviderConfig, TSettings>,
  ) => void | Promise<void>
  runAfterAssistantResponse: (
    context: CoreAgentLoopAfterAssistantResponseContext<TSession, TMessage, TProviderConfig, TSettings>,
  ) => void | Promise<void>
  onError?: (source: 'trigger' | 'afterAssistantResponse', error: unknown) => void
}
...
> {
  ...
  events: [
    CoreAgentLoopAssistantCreatedEvent<CoreAgentLoopAssistantMessage<TToolCall, TContentPart>>,
...
export interface CoreAgentLoopSessionWithMessages<TMessage extends { id: string }> {
  name?: string
  messages: TMessage[]
}

export interface CoreAgentLoopFinalMessageUpdate<
  TMessage extends CoreAgentLoopFinalMessageLike = CoreAgentLoopFinalMessageLike,
> {
...
export function appendAgentLoopTurnToolCallOnce<TToolCall extends CoreAgentLoopToolCallIdentity>(
  turnToolCalls: TToolCall[],
  toolCall: TToolCall,
): boolean {
  if (turnToolCalls.some(existing => existing.id === toolCall.id)) return false
  turnToolCalls.push(toolCall)
  return true
}

export function planAgentLoopToolCallFallback<TToolCall extends CoreAgentLoopToolCallIdentity>(
  existingToolCalls: readonly TToolCall[],
  toolCall: CoreAgentLoopFallbackToolCallLike,
): CoreAgentLoopToolCallFallbackPlan {
  return {
    shouldStartPlaceholder: !existingToolCalls.some(existing => existing.id === toolCall.toolCallId),
...
>(
  ...
  const assistantMessage = createAgentLoopAssistantMessage<TToolCall, TContentPart>(options)
  return {
    assistantMessage,
    events: [
      {
...
export function buildAgentLoopFinalMessageUpdate<TMessage extends CoreAgentLoopFinalMessageLike>(
  message: TMessage,
): CoreAgentLoopFinalMessageUpdate<TMessage> {
  return {
...
export async function emitAgentLoopFinalMessageUpdateWithAdapters<
  ...
  TSession extends CoreAgentLoopSessionWithMessages<TMessage>,
>(
  options: EmitAgentLoopFinalMessageUpdateWithAdaptersOptions<TMessage, TSession>,
): Promise<boolean> {
...
export function appendOrderedPart<TPart extends CoreOrderedPartLike>(parts: TPart[], part: TPart): void {
...
export function planAgentLoopToolContentPartsDispatch<TPart extends CoreOrderedPartLike>(
  turn: Pick<CoreAgentLoopExecutorTurnState<unknown, TPart>, 'orderedParts' | 'hasSentToolParts'>,
  turnIndex: number,
...
export function planAgentLoopTurnContentPersistence<TPart extends CoreOrderedPartLike>(
  turn: Pick<CoreAgentLoopExecutorTurnState<unknown, TPart>, 'orderedParts' | 'toolCalls'>,
  turnIndex: number,
...
export function dispatchAgentLoopToolContentPartsWithAdapters<TPart extends CoreOrderedPartLike>(
  options: DispatchAgentLoopToolContentPartsOptions<TPart>,
): CoreAgentLoopToolContentPartDispatchPlan<TPart> {
  const plan = planAgentLoopToolContentPartsDispatch(options.turn, options.turnIndex)
...
export function persistAgentLoopTurnContentPartsWithAdapters<TPart extends CoreOrderedPartLike>(
  options: PersistAgentLoopTurnContentPartsOptions<TPart>,
): CoreAgentLoopTurnContentPersistencePlan<TPart> {
  const plan = planAgentLoopTurnContentPersistence(options.turn, options.turnIndex)

  for (const part of plan.persistParts) {
    options.store.addMessageContentPart(options.sessionId, options.assistantMessageId, part)
  }
...
export function hasAgentLoopVisibleTurnActivity<TToolCall, TPart extends CoreOrderedPartLike>(
  turn: Pick<CoreAgentLoopExecutorTurnState<TToolCall, TPart>, 'content' | 'toolCalls' | 'hasSentToolParts' | 'orderedParts'>,
): boolean {
  return turn.content.value.length > 0 ||
    turn.toolCalls.length > 0 ||
    turn.hasSentToolParts ||
    turn.orderedParts.some(part => part.type !== 'provider-data')
}

export function getAgentLoopReasoningPlacement<TToolCall, TPart extends CoreOrderedPartLike>(options: {
  turnIndex: number
  accumulatedContent: string
  turn: Pick<CoreAgentLoopExecutorTurnState<TToolCall, TPart>, 'content' | 'toolCalls' | 'hasSentToolParts' | 'orderedParts'>
...
export function structuredToolResult(result: CoreAgentToolResultLike): CoreToolResult {
  return {
...
export function textFromPartialResult(update: CoreToolPartialResultUpdate): string {
...
export function changesFromMetadata(metadata: JsonObject | undefined): CoreToolCallChanges | undefined {
...
export function applyAgentLoopToolMetadata<TToolCall extends CoreAgentLoopToolCallWithMetadata>(
...
export function buildAgentLoopToolPartialStepUpdate<TPartialResult extends CoreToolPartialResultUpdate>(
  update: TPartialResult,
): CoreAgentLoopToolPartialStepUpdate<TPartialResult> {
  return {
    status: 'running',
...
export function isSkillManageToolCall(toolCall: Pick<CoreAgentLoopToolCallForSettlement, 'toolId' | 'toolName'>): boolean {
  return toolCall.toolId === 'skill_manage' || toolCall.toolName === 'skill_manage'
}

export function settleAgentLoopToolCallResult<TToolCall extends CoreAgentLoopToolCallForSettlement>(
  toolCall: TToolCall,
  result: CoreAgentToolResultLike,
  now = Date.now(),
): CoreSettledAgentLoopToolCallResult<TToolCall> {
  const data = objectData(result.data)
  const commandType = result.commandType ?? (typeof data.commandType === 'string' ? data.commandType : undefined)
  ...
  toolCall.rejectionReason = typeof data.rejectionReason === 'string' ? data.rejectionReason : undefined
...
export function startAgentLoopToolExecution<
  ...
  TStepUpdate,
>(
  options: StartAgentLoopToolExecutionOptions<TToolCall, TStepUpdate>,
): void {
  const now = options.now ?? Date.now
  options.toolCall.status = 'executing'
  options.toolCall.startTime = options.toolCall.startTime ?? now()
  options.store.updateMessageToolCalls(options.sessionId, options.assistantMessageId, options.toolCalls)
...
export function settleAgentLoopToolResultWithAdapters<
  ...
  TToolResult = CoreToolResult,
>(
  options: SettleAgentLoopToolResultOptions<TToolCall, TStepUpdate, TToolResult>,
): CoreAgentLoopToolResultSettlementResult<TToolCall> {
  const toolCall = options.toolCalls.find(existing => existing.id === options.toolCallId)
  if (!toolCall) {
    return {
      found: false,
  ...
  const settlement = settleAgentLoopToolCallResult(toolCall, options.result, (options.now ?? Date.now)())
  options.store.updateMessageToolCalls(options.sessionId, options.assistantMessageId, options.toolCalls)
  options.emitter.sendToolResult(toolCall)

  const stepId = options.stepIdsByToolCallId.get(options.toolCallId)
  if (stepId) {
    const presentation = buildAgentLoopToolResultPresentation(toolCall, options.result, settlement.awaitingConfirmation)
...
export function applyAgentLoopToolInputStartWithAdapters<
  ...
  TStepUpdate,
>(
  options: ApplyAgentLoopToolInputStartWithAdaptersOptions<TContentPart, TToolCall, TStepUpdate>,
): string | undefined {
  ...
  options.processor.handleToolInputStart(options.toolCallId, options.toolName, options.turnIndex)
...
export function applyAgentLoopToolInputDeltaWithAdapters<TToolCall extends CoreAgentLoopToolCallForSettlement>(
  processor: Pick<CoreAgentLoopToolInputProcessor<TToolCall>, 'handleToolInputDelta'>,
  toolCallId: string,
...
export function applyAgentLoopToolInputEndWithAdapters<
  ...
  TStepUpdate,
>(
  options: ApplyAgentLoopToolInputEndWithAdaptersOptions<TContentPart, TToolCall, TStepUpdate>,
...
export function applyAgentLoopToolCallFallbackWithAdapters<
  ...
  TStepUpdate,
>(
  options: ApplyAgentLoopToolCallFallbackWithAdaptersOptions<TContentPart, TToolCall, TStepUpdate>,
): TToolCall {
  dispatchAgentLoopToolContentPartsWithAdapters({
    turn: options.turn,
    turnIndex: options.turnIndex,
  ...
  const plan = planAgentLoopToolCallFallback(options.processor.toolCalls, options.toolCall)
  if (plan.shouldStartPlaceholder) {
    options.processor.handleToolInputStart(plan.toolCallId, plan.toolName, options.turnIndex)
...
export function applyAgentLoopToolResultWithAdapters<
  ...
  TToolResult = CoreToolResult,
>(
  options: ApplyAgentLoopToolResultWithAdaptersOptions<TToolCall, TStepUpdate, TToolResult>,
): CoreAgentLoopToolResultSettlementResult<TToolCall> {
...
export function applyAgentLoopToolMetadataWithAdapters<
  ...
  TStepUpdate,
>(
  options: ApplyAgentLoopToolMetadataOptions<TToolCall, TStepUpdate>,
): boolean {
  const stepId = options.stepIdsByToolCallId.get(options.toolCallId)
  if (!stepId) return false

  const toolCall = options.toolCalls.find(existing => 'id' in existing && existing.id === options.toolCallId)
...
export function applyAgentLoopToolPartialResultWithAdapters<
  ...
  TPartialResult extends CoreToolPartialResultUpdate = CoreToolPartialResultUpdate,
>(
  options: ApplyAgentLoopToolPartialResultOptions<TToolCall, TStepUpdate, TPartialResult>,
): boolean {
  const stepId = options.stepIdsByToolCallId.get(options.toolCallId)
  if (!stepId) return false

  options.emitter.sendToolExecutionUpdate(options.toolCallId, stepId, options.update)
  options.emitter.sendStepUpdated(
    stepId,
...
export async function applyAgentLoopProviderDataWithAdapters<TContentPart extends CoreOrderedPartLike>(
  ...
  if (plan.kind === 'provider-data') {
    appendOrderedPart(options.orderedParts, plan.orderedPart as unknown as TContentPart)
    return true
  }
...
export async function applyAgentLoopStreamChunkWithAdapters<
  ...
  TToolResult = CoreToolResult,
>(
  options: ApplyAgentLoopStreamChunkWithAdaptersOptions<TContentPart, TToolCall, TStepUpdate, TToolResult>,
): Promise<unknown> {
  ...
  if (chunk.type === 'tool-input-start' && chunk.toolInputStart) {
    return applyAgentLoopToolInputStartWithAdapters<TContentPart, TToolCall, TStepUpdate>({
      turn: state.turn,
      turnIndex: state.turnIndex,
  ...
  if (chunk.type === 'tool-input-end' && chunk.toolInputEnd) {
    return applyAgentLoopToolInputEndWithAdapters<TContentPart, TToolCall, TStepUpdate>({
      sessionId: options.sessionId,
      assistantMessageId: options.assistantMessageId,
  ...
  if (chunk.type === 'tool-call' && chunk.toolCall) {
    return applyAgentLoopToolCallFallbackWithAdapters<TContentPart, TToolCall, TStepUpdate>({
      sessionId: options.sessionId,
      assistantMessageId: options.assistantMessageId,
      turn: state.turn,
      turnIndex: state.turnIndex,
  ...
  if (chunk.type === 'tool-result' && chunk.toolResult) {
    return applyAgentLoopToolResultWithAdapters<TToolCall, TStepUpdate, TToolResult>({
      state,
      sessionId: options.sessionId,
      assistantMessageId: options.assistantMessageId,
  ...
  if (chunk.type === 'tool-metadata' && chunk.toolMetadata) {
    return applyAgentLoopToolMetadataWithAdapters<TToolCall & CoreAgentLoopToolCallWithMetadata, TStepUpdate>({
      toolCallId: chunk.toolMetadata.toolCallId,
      update: chunk.toolMetadata.update,
      toolCalls: options.processor.toolCalls as Array<TToolCall & CoreAgentLoopToolCallWithMetadata>,
      stepIdsByToolCallId: state.stepIdsByToolCallId,
      emitter: options.emitter,
    })
  }
  ...
  if (chunk.type === 'finish') {
    return applyAgentLoopFinishChunkWithAdapters({
      ...
      sendContextSizeUpdate: inputTokens => options.emitter.sendContextSizeUpdate(inputTokens),
      persistTurnContentParts: options.persistTurnContentParts,
      createTurnState: options.createTurnState,
      sendContinuation: turnIndex => options.emitter.sendContinuation(turnIndex),
    })
  }
...
export async function executeAgentLoopStreamLifecycleWithAdapters<
  ...
  TSupportedPrepared extends TPrepared,
>(
  options: ExecuteAgentLoopStreamLifecycleWithAdaptersOptions<TPrepared, TSupportedPrepared>,
...
export function applyAgentLoopTextChunkWithAdapters<TContentPart extends CoreOrderedPartLike>(
  options: ApplyAgentLoopTextChunkWithAdaptersOptions<TContentPart>,
): boolean {
  const displayContent = options.handleTextChunk(options.text, options.content, options.turnIndex)
...
export function applyAgentLoopReasoningChunkWithAdapters<TContentPart extends CoreOrderedPartLike>(
  ...
  options.handleReasoningChunk(options.reasoning, options.turn.reasoning, options.turnIndex, placement)
  if (placement === 'inline') {
...
export function lastUserMessageText(historyMessages: CoreHistoryMessageWithContent[]): string {
...
>(input: {
  ...
  skillManageCalled: boolean
  prepared: CorePreparedToolNames
}): CoreAgentLoopPostResponseContexts<TSession, TMessage, TProviderConfig, TSettings> | null {
  if (!input.session || !input.lastAssistantMessage) return null

  const triggerContext: CoreAgentLoopPostResponseTriggerContext<TSession, TMessage, TProviderConfig, TSettings> = {
...
export function runAgentLoopPostResponseHooksWithAdapters<
  ...
  TSettings,
>(
  options: RunAgentLoopPostResponseHooksWithAdaptersOptions<TSession, TMessage, TProviderConfig, TSettings>,
): CoreAgentLoopPostResponseContexts<TSession, TMessage, TProviderConfig, TSettings> | null {
  const contexts = buildAgentLoopPostResponseContexts<TSession, TMessage, TProviderConfig, TSettings>({
  ...
  const run = (
    ...
    try {
      Promise.resolve(callback()).catch(error => options.onError?.(source, error))
    } catch (error) {
  ...
  run('afterAssistantResponse', () => options.runAfterAssistantResponse(contexts.afterAssistantResponseContext))
  ...
  if (input.usage) {
    accumulatedUsage = accumulatedUsage
      ? {
          ...
          outputTokens: accumulatedUsage.outputTokens + input.usage.outputTokens,
...
export function isAgentLoopToolCallsFinishReason(finishReason: string | undefined): boolean {
...
export function applyAgentLoopFinishChunkWithAdapters<TTurn>(
  ...
  if (plan.lastTurnUsage) {
    ...
    options.updateStepsUsageByTurn?.(options.state.turnIndex, plan.lastTurnUsage)
  }
  ...
  options.state.createNewAssistantOnNextTurnStart = plan.createNewAssistantOnNextTurnStart
