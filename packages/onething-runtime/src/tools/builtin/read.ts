...
export function createReadTool(adapters: ReadToolAdapters = {}): Tool.Info<typeof ReadParameters, ReadMetadata> {
  return Tool.define<typeof ReadParameters, ReadMetadata>('read', {
    ...
    async analyze(args, ctx) {
      ...
      const resolvedPath = resolveReadPath(args.path, ctx.workingDirectory, adapters)
    ...
    async execute(args, ctx) {
      ...
      const defaultWorkingDirectory = adapters.getDefaultWorkingDirectory?.()
      const resolvedPath = await checkCoreFileAccess(args.path, ctx, 'Read file', {
        targetType: 'file',
        defaultWorkingDirectory,
      })
      throwIfAborted()
