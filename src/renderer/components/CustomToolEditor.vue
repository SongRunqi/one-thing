...
type CustomToolParameterType = 'string' | 'number' | 'boolean' | 'object' | 'array'
...
watch(() => props.tool, (tool) => {
  if (tool) {
    ...
    form.parameters = tool.parameters.map((p: any) => ({
      name: p.name,
      type: p.type,
    ...
    if (exec?.type === 'bash') {
      ...
      form.bashCommand = exec.command || ''
      form.bashTimeout = exec.timeout || 30000
    } else if (exec?.type === 'http') {
      form.executionType = 'http'
      form.httpMethod = (exec.method || 'GET') as typeof form.httpMethod
      form.httpUrl = exec.url || ''
      form.httpHeaders = exec.headers ? JSON.stringify(exec.headers, null, 2) : ''
      form.httpBody = exec.bodyTemplate || ''
    }
  }
}, { immediate: true })
