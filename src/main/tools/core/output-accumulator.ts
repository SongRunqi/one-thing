...
function stripAnsi(text: string): string {
  // Covers the common CSI/OSC ANSI escape forms used in CLI output.
...
function sanitizeOutput(text: string): string {
  ...
  return Array.from(stripped)
    .filter((char) => {
      ...
      // Avoid Unicode format/control-like chars that can break width/rendering libraries.
...
function trimToLastBytes(text: string, maxBytes: number): string {
  ...
  // Avoid starting in the middle of a UTF-8 continuation byte.
...
/**
 * Incrementally accumulates command output with bounded memory.
 *
 * The accumulator keeps a sanitized tail for UI/result display and starts
 * persisting the full sanitized output to a temp file once output exceeds the
 * configured line/byte limits.
 */
