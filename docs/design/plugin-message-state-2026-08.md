...
### 6.1 走查实录(2026-08-09 第一轮)

现象:重启后只有**最后一条**消息有徽标,倒数第二条(装完 1.0.2 那一轮生成的)
没有。前两条是 1.0.1 时代的消息 —— 按设计就该空(插件不在场的流不追认)。

定位(逐条排除,不靠猜):记录文件在盘上、内容 `v:1` 正常 → 写路径与 lifetime
闸门都对;日志无 handler 报错、无级联删除事件;`getDeclaredUiSlots` 的 1s TTL
缓存在那一刻是**刚建的**(装完之后才有插件创建 API),读的是 1.0.2 的 manifest,
闸门必然开着。于是唯一剩下的可能是:**那一轮跑的根本不是 1.0.2 的代码**。

病根在插件重载,不在消息态:热重载令牌是宿主 ESM cache-buster 的取值,而
`uninstallPlugin` 把令牌删回 0、`installPlugin`/`updatePlugin` 根本不动它 ——
卸载 1.0.1 再装 1.0.2 拿回了用过的号,`import` 命中模块缓存,进程里跑的仍是
1.0.1 的纯内存实现(所以徽标当场有、盘上没有)。已修:令牌改进程内全局单调,
装/更新/回滚/重新启用各发一个新号(`packages/core/plugins/manager.ts`,回归测试
在 `install-lifecycle.test.ts`)。

**这条 bug 的影响面远大于本期**:此前每一次在线更新插件,新代码都要等下次
重启才生效,而版本号当场就变了 —— 版本号与行为不一致,还没人报错。
