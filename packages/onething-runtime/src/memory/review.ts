...
export type MemoryReviewCandidate = CoreMemoryReviewCandidate & { target: MemoryReviewTarget }
...
export const MEMORY_REVIEW_LONG_TERM_FILENAME = CORE_HERMES_LONG_TERM_MEMORY_FILENAME

function asCoreMessages(messages: MemoryReviewMessageLike[]): CoreMemoryReviewMessage[] {
...
export function parseMemoryReviewModelResult(value: string): MemoryReviewModelResult | null {
  return parseCoreMemoryReviewModelResult(value) as MemoryReviewModelResult | null
}
...
export async function readPlainReviewFile(
  workspace: MemoryWorkspace,
  target: CorePlainReviewTarget,
): Promise<CoreMemoryReviewPlainFile & {
  absolutePath: string
...
export async function runMemoryReview<TProvider>(options: {
  ...
  force?: boolean
  lastReviewedTurn?: number
  resolveProvider: () => CoreSoulMemoryReviewProviderRef<TProvider> | Promise<CoreSoulMemoryReviewProviderRef<TProvider>>
  generateReview: (input: CoreSoulMemoryReviewGenerateInput<TProvider>) => string | Promise<string>
  ...
  return coreRunSoulMemoryReview<TProvider>({
    ...
    applyCandidate: (candidate, minConfidence) => applyMemoryReviewCandidate({
      workspace: options.workspace,
      candidate,
      minConfidence,
