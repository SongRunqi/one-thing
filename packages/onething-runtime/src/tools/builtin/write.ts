...
export function createWriteTool(adapters: WriteToolAdapters): Tool.Info<typeof WriteParameters, WriteMetadata> {
  return Tool.define<typeof WriteParameters, WriteMetadata>('write', {
    ...
    async analyze(args, ctx) {
      ...
      const resolvedPath = resolveWritePath(args.path, ctx.workingDirectory, adapters)
    ...
    async execute(args, ctx) {
      ...
      const resolvedPath = resolveWritePath(inputPath, ctx.workingDirectory, adapters)
