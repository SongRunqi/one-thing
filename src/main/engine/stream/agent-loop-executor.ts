...
import { IPC_CHANNELS, type ContentPart, type Step, type ToolCall, type ToolResult } from '../../../shared/ipc.js'
...
export function runAgentLoopPostResponseHooks(options: {
  ...
  runAgentLoopPostResponseHooksWithAdapters<ChatSession, ChatMessage, typeof options.state.ctx.providerConfig, typeof options.state.ctx.settings>({
    ...
    runTriggerContext: context =>
      triggerManager.runPostResponse(context as Parameters<typeof triggerManager.runPostResponse>[0]),
    ...
    onError(source, error) {
      ...
      console.error('[AgentLoopExecutor] Plugin after-response hook failed:', error)
    },
  })
}
...
export async function applyAgentLoopStreamChunk(
  ...
  await coreApplyAgentLoopStreamChunkWithAdapters<ContentPart, ToolCall, Partial<Step>, ToolResult>({
    state,
    chunk,
    sessionId: state.ctx.sessionId,
    assistantMessageId: state.ctx.assistantMessageId,
    ...
    handleTextChunk: (text, content, turnIndex) => state.processor.handleTextChunk(text, content, turnIndex),
    handleReasoningChunk: (reasoning, accumulator, turnIndex, placement) =>
      state.processor.handleReasoningChunk(reasoning, accumulator, turnIndex, placement),
    applyProviderData: options => applyOnethingAgentLoopProviderData({
      ...options,
      saveMediaImage,
    ...
    updateStepsUsageByTurn: (turnIndex, usage) => {
      store.updateStepsUsageByTurn(state.ctx.sessionId, state.ctx.assistantMessageId, turnIndex, usage)
    },
    now: Date.now,
  })
}
...
export async function executeAgentLoopStreamGeneration(
  ...
  >({
    prepareRuntime: () => buildAgentLoopRuntimeFromStreamContext(ctx, historyMessages, { emitter }),
    isRuntimeSupported: (prepared): prepared is Extract<BuildAgentLoopStreamRuntimeResult, { supported: true }> =>
      prepared.supported,
    unsupportedReason: prepared => prepared.supported ? 'Unsupported agent loop runtime' : prepared.reason,
    ...
    runPostResponseHooks: prepared => runAgentLoopPostResponseHooks({
      state,
      prepared,
      historyMessages,
    }),
    isAbortError: error => error.name === 'AbortError' || ctx.abortSignal.aborted,
    ...
    emitFinalAssistantMessageUpdate: () => emitFinalAssistantMessageUpdate(state),
