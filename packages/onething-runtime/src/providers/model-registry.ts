...
export type OnethingModelsDevResponse = Record<string, OnethingModelsDevProvider>
...
export type OnethingProviderModelConfigs = Record<string, OnethingProviderModelConfig | undefined>

export interface OnethingModelRegistryQueryOptions {
  getFallbackModelsForProvider?(providerId: string): OnethingOpenRouterModel[]
  getFallbackModel?(modelId: string, providerId?: string): OnethingOpenRouterModel | undefined
}
...
export interface OnethingModelRegistryRefreshAdapters<
  TSettings extends OnethingModelRegistrySettingsLike = OnethingModelRegistrySettingsLike,
> {
...
export interface GetOnethingModelsWithCapabilitiesAdapters {
  ...
  getConfiguredCodexModelSelection(): OnethingConfiguredModelSelection | undefined
...
export interface FetchOnethingGitHubCopilotModelsWithAuthOptions<TModel = unknown> {
  providerId?: string
  getToken(providerId: string): Promise<OnethingAccessTokenLike | null | undefined> | OnethingAccessTokenLike | null | undefined
  fetchCopilotModels(accessToken: string): Promise<TModel[]> | TModel[]
  missingTokenError?: string
}

export async function fetchOnethingGitHubCopilotModelsWithAuth<TModel = unknown>(
  ...
  if (!token?.accessToken) {
    throw new Error(options.missingTokenError ?? 'Not logged in to GitHub Copilot')
  }
...
export function getOnethingModelsDevProviderId(providerId: string): string {
  return Object.entries(ONETHING_PROVIDER_MAPPING).find(([, mappedId]) => mappedId === providerId)?.[0] || providerId
}
...
export function mergeOnethingModelsById<TModel extends { id: string }>(...groups: TModel[][]): TModel[] {
...
export function getConfiguredOnethingModelIds(config?: OnethingConfiguredModelSelection): string[] {
...
export function acpAgentToOnethingOpenRouterModel(agent: OnethingACPAgentModelLike): OnethingOpenRouterModel {
  return {
    id: agent.id,
...
function isOnethingGitHubCopilotProviderId(
  ...
  return providerIdMatches(providerId, aliases?.githubCopilot, ['github-copilot'])
}
...
function getConfiguredCodexFallbackModelsWithAdapters(
  adapters: Pick<GetOnethingModelsWithCapabilitiesAdapters, 'getConfiguredCodexModelSelection' | 'getCodexFallbackModels'>,
): OnethingOpenRouterModel[] {
...
async function getCachedCodexModelsWithFallbacks(
  adapters: Pick<GetOnethingModelsWithCapabilitiesAdapters, 'getModelsForProvider' | 'getConfiguredCodexModelSelection' | 'getCodexFallbackModels'>,
  includeDefaultFallback = false,
): Promise<OnethingOpenRouterModel[]> {
  const registryModels = await adapters.getModelsForProvider('codex')
  const groups = [registryModels, getConfiguredCodexFallbackModelsWithAdapters(adapters)]
...
export async function getOnethingModelsWithCapabilities(
  ...
  try {
    if (isOnethingGitHubCopilotProviderId(request.providerId, adapters.providerIds)) {
      try {
        const copilotModels = await adapters.fetchCopilotModels()
        return {
          success: true,
          models: copilotModels.map(model => copilotModelInfoToOnethingOpenRouterModel(model)),
        }
      } catch (error) {
        adapters.logger?.warn?.('[Models] Failed to fetch Copilot models:', error instanceof Error ? error.message : String(error))
        const registryModels = await adapters.getModelsForProvider('github-copilot')
    ...
    if (isOnethingCodexProviderId(request.providerId, adapters.providerIds)) {
      ...
      try {
        ...
        return {
          success: true,
          models: mergeOnethingModelsById(models, getConfiguredCodexFallbackModelsWithAdapters(adapters)),
        }
      } catch (error) {
        adapters.logger?.warn?.('[Models] Failed to fetch Codex models, using fallback:', error instanceof Error ? error.message : String(error))
        return {
          success: true,
    ...
    adapters.logger?.error?.('[Models] Failed to get models with capabilities:', error)
...
>(
  providerId: string,
  models: OnethingOpenRouterModel[],
  adapters: Pick<OnethingModelRegistryRefreshAdapters<TSettings>, 'getSettings' | 'saveSettings' | 'now' | 'logger'>,
): void {
  const settings = adapters.getSettings()
  const providerConfig = settings.ai.providers[providerId] || {}
  providerConfig.models = createOnethingModelEntriesFromOpenRouterModels(providerId, models)
...
>(
  ...
  adapters.logger?.log?.(`[ModelRegistry] Refreshing models for provider: ${providerId}`)

  if (providerId === 'codex') {
    adapters.logger?.log?.('[ModelRegistry] Codex models are loaded from the authenticated Codex backend')
    return
  }
  ...
  adapters.saveSettings(settings)
  adapters.logger?.log?.(`[ModelRegistry] Saved ${Object.keys(models).length} models for provider ${providerId}`)
}
...
>(
  ...
  for (const providerId of providerIds) {
    ...
    if (!models) {
      adapters.logger?.warn?.(`[ModelRegistry] No models.dev data for provider: ${providerId}`)
      continue
    }
    ...
    adapters.logger?.log?.(`[ModelRegistry]   ${providerId}: ${Object.keys(models).length} models`)
  }
...
export function createOnethingModelEntriesFromModelsDev(
  ...
  for (const [modelId, model] of Object.entries(devProvider.models)) {
    models[modelId] = modelsDevModelToOnethingCapabilityEntry(model, providerId)
  }
  return models
}
...
export function createOnethingModelEntriesFromOpenRouterModels(
  ...
  for (const model of models) {
    entries[model.id] = openRouterModelToOnethingCapabilityEntry(model, providerId)
  }
  return entries
}

export function sortOnethingModels(models: OnethingOpenRouterModel[]): OnethingOpenRouterModel[] {
...
export function getOnethingModelsForProvider(
  ...
  if (providerId === 'claude-code') {
    entries = entries.filter(entry => {
      ...
      return CLAUDE_CODE_MODEL_PATTERNS.some(pattern => lower.includes(pattern))
    })
  }

  return sortOnethingModels(entries.map(onethingCapabilityEntryToOpenRouterModel))
}
...
export function onethingModelSupportsTools(
  ...
  const override = getCapabilityOverride(providers, modelId, providerId, 'tools')
  ...
  if (['image', 'vision-preview', 'dall-e', 'imagen', 'ocr', 'embedding', 'asr'].some(pattern => lower.includes(pattern))) {
    return false
  }
  return true
}
...
export function onethingModelSupportsReasoning(
  ...
  const override = getCapabilityOverride(providers, modelId, providerId, 'reasoning')
  if (override !== undefined) return override
  ...
  const lower = modelId.toLowerCase()
  return ['reasoner', 'o1', 'o3', 'thinking'].some(pattern => lower.includes(pattern))
}
...
export function onethingModelSupportsReasoningSync(
  ...
  if (['reasoner', 'o1-', 'o3-', '-o1', '-o3', 'thinking'].some(pattern => lower.includes(pattern))) return true
  return false
}
...
export function onethingModelSupportsImageGeneration(
  ...
  const override = getCapabilityOverride(providers, modelId, providerId, 'imageOutput')
  if (override !== undefined) return override

  const lower = modelId.toLowerCase()
  if (lower.includes('gemini') && lower.includes('image')) return true
  if (['dall-e', 'dalle', 'imagen', 'gpt-image', 'flux', 'stable-diffusion', 'midjourney'].some(pattern => lower.includes(pattern))) {
    return true
  }
...
export function getOnethingModelCacheStatus(
  ...
  if (providers) {
    for (const providerId of Object.keys(providers)) {
      ...
      total += count
      if (config?.modelsLastFetched && config.modelsLastFetched > latest) latest = config.modelsLastFetched
    }
  }
