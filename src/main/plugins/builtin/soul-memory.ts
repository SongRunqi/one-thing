...
import {
  searchMarkdownMemoryChunks as runtimeSearchMarkdownMemoryChunks,
} from '@onething/runtime/memory/search'
import {
...
import {
  buildSoulMemoryPromptContext as runtimeBuildSoulMemoryPromptContext,
} from '@onething/runtime/memory/prompt-context'
import {
  appendMemoryNote as runtimeAppendMemoryNote,
} from '@onething/runtime/memory/append'
import {
...
import {
  runMemoryFlush as runtimeRunMemoryFlush,
} from '@onething/runtime/memory/flush'
import {
  runMemoryActiveMemoryRecall as runtimeRunMemoryActiveMemoryRecall,
} from '@onething/runtime/memory/active-memory'

export { SOUL_MEMORY_PLUGIN_ID } from '../../memory/workspace.js'
export type { MemoryWorkspace, ResolvedSoulMemorySettings } from '@onething/runtime/memory/types'
...
function markIndexDirty(reason: string, metadata?: Record<string, unknown>): void {
  ...
  logMemoryDiagnostic(coreBuildSoulMemoryIndexDirtyDiagnostic({
    reason,
    state,
    metadata: {
      ...(metadata || {}),
...
function ensureIndexWatcher(workspace: MemoryWorkspace): void {
  ...
  try {
    indexWatcher = watchDirectoryRecursive(workspace.memoryDir, (eventType, filename) => {
      if (!filename) return
      const relativePath = normalizeMemoryRelativePath(joinPaths('memory', filename.toString()))
...
function refreshIndexStatus(database: MemoryDatabase): IndexStatus {
  ...
  lastStatus = coreRefreshSoulMemoryIndexStatus(lastStatus, counts, getFtsTokenizer())
  return lastStatus
}
...
async function syncIndex(options: { settings?: AppSettings; force?: boolean; agentId?: string } = {}): Promise<IndexStatus> {
  ...
  try {
    ...
    const workPlan = corePlanSoulMemoryIndexSyncWork(freshness, { force: options.force })
    ...
    for (const file of workPlan.filesToIndex) {
      const result = await runtimeIndexMemoryFile({
        ...
        file,
        knownStat: file,
        embedTexts: (settings, values) => embedTexts({ settings: settings ?? getSettings(), values }),
...
async function searchMemory(options: {
  ...
  const database = getDb(workspace)
  const limit = coreNormalizeSoulMemorySearchLimit(options.limit, workspace.settings.search.maxResults)
...
async function migrateCanonicalMemoryIfNeeded(workspace: MemoryWorkspace, settings?: AppSettings): Promise<void> {
  ...
  try {
    ...
    const pendingCandidates = coreBuildSoulMemoryPendingCaptureMigrationCandidates(pending) as CaptureCandidate[]
  ...
  try {
    ...
    if (canonicalRows.length > 0) {
      await upsertGraphCandidates({
        settings,
        workspace,
        candidates: coreBuildSoulMemoryCanonicalGraphMigrationCandidates(canonicalRows) as CaptureCandidate[],
...
const extractLegacyMemoryCandidates = coreExtractLegacyMemoryCandidates as (content: string) => CaptureCandidate[]
...
const parseDailyNoteCaptureResult = coreParseDailyNoteCaptureResult as (value: string) => DailyNoteCaptureResult | null
...
async function applyDailyNoteCaptureActions(options: {
  ...
  return runtimeApplyDailyNoteCaptureActions({
    ...
    logDiagnostic: event => logMemoryDiagnostic(event),
    onIndexableWrite: target => {
      markIndexDirty('daily-note-actions', { relativePath: target.relativePath })
      scheduleIndexSync({
        settings: options.settings,
        agentId: workspace.agentId,
...
async function runMemoryCapture(api: PluginAPI, context: AfterAssistantResponseContext): Promise<void> {
  ...
  await runtimeRunMemoryCapture<MemoryToolProvider>({
    ...
    generateCapture: ({ provider, system, prompt, temperature, maxTokens }) => generateChatResponse(
      provider.providerId,
      provider.config,
      [
...
async function applyMemoryReviewCandidate(
  ...
  return runtimeApplyMemoryReviewCandidate({
    ...
    minConfidence,
    onIndexableWrite: target => {
      markHermesFileMemoryChanged(workspace, target.relativePath, 'memory-review-candidate')
    },
  })
}
...
async function runMemoryReview(
  ...
  await runtimeRunMemoryReview<MemoryToolProvider>({
    ...
    force: options.force,
    lastReviewedTurn: api.store.get<number>(runtimeMemoryReviewLastTurnKey(agentId, context.sessionId)),
    resolveProvider: async () => {
    ...
    generateReview: ({ provider, system, prompt, temperature, maxTokens }) => generateChatResponse(
      provider.providerId,
      provider.config,
      [
        { role: 'system', content: system },
...
function buildPublicDreamingStatus(workspace: MemoryWorkspace): MemoryDreamingStatus {
  ...
  return coreBuildSoulMemoryPublicDreamingStatus({
    ...
    model: coreFormatSoulMemoryToolModelRef(asCoreProviderSelectionSettings(getSettings())),
    sources: ['daily'],
    resolveNextRunAt: (frequency, timezone, from) => nextCronRunAt(frequency, timezone, from),
    scheduled,
    store: pluginStore,
    runtimeStatus: lastStatus,
...
export async function getSoulMemoryOverview(agentId = DEFAULT_AGENT_ID): Promise<MemoryOverview> {
  ...
  const files = await runtimeListManagedMemoryFiles(workspace) as MemoryManagedFile[]
...
export async function runSoulMemoryDreamingNow(agentId?: string): Promise<DreamingRunResult | null> {
  return coreRunSoulMemoryManualDreamingWithAdapters<DreamingRunResult>({
    ...
    runWithPluginApi: async plan => runMemoryDreamingSweep(activeSoulMemoryPluginApi!, {
      agentId: plan.agentId,
      reason: plan.reason,
      force: plan.force,
    }),
    runWithScheduler: plan => getScheduler().runNow(SCOPED_DREAMING_SCHEDULER_TASK_ID, {
      reason: plan.reason,
      force: plan.force,
    }) as Promise<{ ok: boolean; error?: string; result?: DreamingRunResult | null }>,
  })
}
...
const parseDreamingOutput = coreParseDreamingOutput as (text: string) => DreamingMemoryResult
...
async function collectDailyDreamingSources(workspace: MemoryWorkspace): Promise<DreamingSource[]> {
  return runtimeCollectDailyDreamingSources(workspace) as Promise<DreamingSource[]>
}

async function collectDreamingSources(workspace: MemoryWorkspace): Promise<{ sources: DreamingSource[] }> {
  return runtimeCollectDreamingSources(workspace) as Promise<{ sources: DreamingSource[] }>
}
...
function asCoreProviderSelectionSettings(settings: AppSettings): CoreSoulMemoryProviderSelectionSettings {
...
async function resolveMemoryToolProvider(settings: AppSettings, purpose: string): Promise<MemoryToolProvider> {
  const selection = coreResolveSoulMemoryToolProviderSelection(asCoreProviderSelectionSettings(settings)) as MemoryToolProviderSelection
  const authContext = await resolveProviderAuth(selection.providerId, selection.config)
  return coreCreateSoulMemoryToolProvider(selection, authContext, purpose) as MemoryToolProvider
}
...
async function applyDreamingMemoryActions(
  ...
  return runtimeApplyDreamingMemoryActions({
    ...
    logDiagnostic: event => logMemoryDiagnostic(event),
    onIndexableWrite: target => {
      markHermesFileMemoryChanged(workspace, target.relativePath, 'dreaming-memory-actions')
    },
...
async function runMemoryDreamingSweep(api: PluginAPI, options: {
  ...
  dreamingRunInFlight = (async () => {
    ...
    return await runtimeRunMemoryDreamingSweep<MemoryToolProvider>({
      ...
      force: options.force,
      now: options.now,
      getNextRunAt: (dreaming, from) => coreGetSoulMemoryDreamingNextRunStatus(
        dreaming,
        nextCronRunAt,
        from,
      ),
      ...
      generateDreaming: ({ provider, system, prompt, temperature, maxTokens }) => generateChatResponse(
        provider.providerId,
        provider.config,
        [
          { role: 'system', content: system },
...
function buildDreamingStatus(api: PluginAPI, workspace: MemoryWorkspace): ReturnType<typeof coreBuildSoulMemoryPublicDreamingStatus> {
...
function formatDreamingStatus(api: PluginAPI, workspace: MemoryWorkspace): string {
  return coreFormatSoulMemoryDreamingStatus(buildDreamingStatus(api, workspace))
}

function buildMemoryReviewStatus(api: PluginAPI, workspace: MemoryWorkspace, sessionId: string): ReturnType<typeof coreBuildSoulMemoryReviewStatusWithAdapters> {
...
function formatMemoryReviewStatus(api: PluginAPI, workspace: MemoryWorkspace, sessionId: string): string {
  return coreFormatSoulMemoryReviewStatus(buildMemoryReviewStatus(api, workspace, sessionId))
}
...
async function handleDreamingCommand(api: PluginAPI, args: string, ctx: PluginCommandContext): Promise<void> {
  await coreHandleSoulMemoryDreamingCommand({
    args,
    ctx,
    getStatus: async () => formatDreamingStatus(api, await ensureWorkspace(getSettings())),
    ...
    runNow: () => api.scheduler.runNow(DREAMING_SCHEDULER_TASK_ID, { reason: 'manual', force: true }) as Promise<{
      ok: boolean
      error?: string
...
async function activeMemoryRecall(options: {
  ...
  return runtimeRunMemoryActiveMemoryRecall<MemoryToolProvider>({
    ...
    messages: session?.messages ?? [],
    settings: resolved,
    sessionDisabled: Boolean(options.api.store.get<boolean>(`active-memory-disabled:${options.sessionId}`)),
    search: ({ query, limit }) => searchMemory({
      settings: options.settings,
      agentId,
      query,
      limit,
    }),
    resolveProvider: () => resolveMemoryToolProvider(options.settings, 'Active Memory'),
    providerId: provider => provider.providerId,
    providerModel: provider => String(provider.config.model || 'none'),
    generateFilter: ({ provider, system, prompt, temperature, maxTokens }) => generateChatResponse(
      provider.providerId,
      provider.config,
      [
        { role: 'system', content: system },
...
export const __testing = {
  ...
  dailyNoteExtractionSystemPrompt: CORE_SOUL_MEMORY_DAILY_NOTE_EXTRACTION_SYSTEM_PROMPT,
  ...
  resolveMemoryToolProviderSelection: coreResolveSoulMemoryToolProviderSelection as unknown as (
    settings: AppSettings,
...
export default function soulMemoryPlugin(api: PluginAPI): void {
  ...
  api.registerPromptContextProvider(CORE_SOUL_MEMORY_PROMPT_CONTEXT_PROVIDER_ID, async context => {
    ...
    return runtimeBuildSoulMemoryPromptContext({
      workspace,
      sessionId,
      userTurnCount: session?.messages.filter(message => message.role === 'user').length,
  ...
  api.beforeContextCompact(CORE_SOUL_MEMORY_BEFORE_CONTEXT_COMPACT_HOOK_ID, async context => {
    ...
    const result = await runtimeRunMemoryFlush<MemoryToolProvider>({
      ...
      generateFlush: ({ provider, system, prompt, temperature, maxTokens }) => generateChatResponse(
        provider.providerId,
        provider.config,
        [
          { role: 'system', content: system },
  ...
  api.registerTool({
    ...
    async execute(args, ctx) {
      ...
      return coreHandleSoulMemoryMemoryTool({
        ...
        add: (target, content) => addHermesMemoryEntry({ workspace, target, content }),
        replace: (target, oldText, newText, replaceAll) => replaceHermesMemoryText({
          workspace,
          target,
          oldText,
          newText,
        ...
        remove: (target, text, removeAll) => removeHermesMemoryText({
          workspace,
          target,
          text,
          removeAll,
  ...
  api.registerCommand(activeMemoryCommand.name, {
    ...
    async handler(args, ctx) {
      await coreHandleSoulMemoryActiveMemoryCommand({
        ...
        getSettings,
        saveSettings,
        getGlobalEnabled: settings => settings.general.soulMemory?.activeMemory?.enabled !== false,
  ...
  api.registerCommand(memoryCommand.name, {
    ...
    async handler(args, ctx) {
      await coreHandleSoulMemoryMemoryCommand({
        args,
        ctx,
        handleDreaming: commandArgs => handleDreamingCommand(api, commandArgs, ctx),
        handleReview: commandArgs => handleMemoryReviewCommand(api, commandArgs, ctx),
        handleCapture: rest => coreHandleSoulMemoryCaptureCommand({
          rest,
          ctx,
          getStatus: () => {
        ...
        get: async filePath => {
          ...
          return coreHandleSoulMemoryCommandGet({
            ...
            getCanonical: path => getCanonicalMemoryByIdOrKey(workspace, path),
            readFileExcerpt: path => readMemoryFileExcerpt({ workspace, inputPath: path }),
          })
        },
        ...
        index: () => syncIndex({ agentId: resolveSessionAgentId(ctx.sessionId), force: true }),
        status: async () => {
          const agentId = resolveSessionAgentId(ctx.sessionId)
          const workspace = await ensureWorkspace(getSettings(), agentId)
  ...
  api.registerCommand(soulCommand.name, {
    ...
    async handler(args, ctx) {
      await coreHandleSoulMemorySoulCommand({
        args,
        ctx,
        getWorkspace: () => ensureWorkspace(getSettings(), resolveSessionAgentId(ctx.sessionId)),
        readSoulContent: (workspace, maxChars) => readLimited(workspace.soulPath, maxChars),
      })
    },
  })
