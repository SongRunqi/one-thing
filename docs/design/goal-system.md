...
> ⚠️ **本文档大半已被取代,请勿据此实现。** 先读:
> - **`goal-system-v2.md`** — 推翻了本文 §0/§5a/§7 的「计数器盲推」范式,改为
>   「继续是模型的声明,停止是默认」。模型可设状态由 complete/blocked 改为
>   complete/paused;`blocked` 收为系统专用。
> - **`goal-system-v3.md`** — 推翻了本文 §3 的「一会话只存一个 goal」存储形态,
>   改为保留历史记录。
>
> 本文仍然有效的部分:挂接点映射、记账/预算机制、安全阀(失速检测、错误熔断、
> 中止处理)、UI 入口。已知失效点:§5c 的 `continuationCount` 已从「续跑寿命额度」
> 演化为失速检测器(连续无进展计数);§9/§11.5 称默认 token 上限 1,000,000,实际
> 代码默认无上限,该处风险论证随之失效。
>
