...
import { getUserSkillsPath, loadAllSkills } from './loader.js'
...
function ensureUserSkillsRoot(): string {
  const root = getUserSkillsPath()
...
function resolveCreateSkillDir(name: string, category: string | undefined): string {
...
function visibleSkills(options: SkillManageOptions = {}): SkillDefinition[] {
  return loadAllSkills(options.workingDirectory)
}
...
function createPlan(args: SkillManageArgs, options: SkillManageOptions): MutationPlan {
  ...
  const skillDir = resolveCreateSkillDir(name, category)
...
function deletePlan(args: SkillManageArgs, options: SkillManageOptions): MutationPlan {
  ...
  return {
    ...
    apply: () => {
      ...
      const root = skill.rootPath ?? getUserSkillsPath()
