...
describe('agent provider runtime factory', () => {
  ...
  it('creates supported agent providers from runtime config', () => {
    ...
    const claude = createAgentProviderFromRuntime('claude', {
      apiKey: 'key',
      baseUrl: 'https://anthropic.test/v1',
    }, {
      fetchImpl: emptyFetch,
    })
    const gemini = createAgentProviderFromRuntime('gemini', {
      apiKey: 'key',
      baseUrl: 'https://gemini.test/v1beta',
    }, {
      fetchImpl: emptyFetch,
    })
    const githubCopilot = createAgentProviderFromRuntime('github-copilot', {
      apiKey: 'github-token',
      baseUrl: 'https://copilot.test',
    }, {
      fetchImpl: emptyFetch,
    })
    const claudeCode = createAgentProviderFromRuntime('claude-code', {
  ...
  it('passes DeepSeek model token limits from runtime metadata into provider capabilities', () => {
    const provider = createAgentProviderFromRuntime('deepseek', {
      apiKey: 'key',
  ...
  it('streams GitHub Copilot through the agent runtime with a exchanged Copilot token', async () => {
    ...
    const fetchImpl: typeof globalThis.fetch = async (input, init) => {
      ...
      if (url === 'https://api.github.com/copilot_internal/v2/token') {
        return new Response(JSON.stringify({ token: 'copilot-token', expires_in: 1800 }), {
          status: 200,
    ...
    const provider = createAgentProviderFromRuntime('github-copilot', {
      apiKey: 'github-token',
      baseUrl: 'https://copilot.test',
      model: 'gpt-4o',
    }, { fetchImpl })

    if (!provider?.streamTurn) throw new Error('GitHub Copilot runtime did not create a stream provider')
  ...
  it('streams Claude Code through the agent runtime with OAuth headers', async () => {
    ...
    const provider = createAgentProviderFromRuntime('claude-code', {
      apiKey: 'claude-oauth-token',
      baseUrl: 'https://claude-code.test/v1',
      model: 'claude-sonnet-4',
    }, { fetchImpl })

    if (!provider?.streamTurn) throw new Error('Claude Code runtime did not create a stream provider')
    ...
    expect(requestHeaders.get('authorization')).toBe('Bearer claude-oauth-token')
  ...
  it('creates custom OpenAI-compatible agent providers from runtime config', () => {
    const provider = createAgentProviderFromRuntime('custom-local-openai', {
      apiKey: 'key',
  ...
  it('creates custom Anthropic-compatible agent providers from runtime config', () => {
    const provider = createAgentProviderFromRuntime('custom-local-anthropic', {
      apiKey: 'key',
    ...
    expect(isAgentProviderRuntimeSupported('custom-local-anthropic')).toBe(true)
  ...
  it('allows additional provider runtimes to register without changing the factory', () => {
    const unregister = registerAgentProviderRuntime('plugin-agent', config => ({
    ...
    try {
      ...
      expect(createAgentProviderFromRuntime('plugin-agent', { model: 'm1' })?.id)
        .toBe('plugin-agent:m1')
      expect(() => registerAgentProviderRuntime('plugin-agent', () => ({ id: 'duplicate' })))
